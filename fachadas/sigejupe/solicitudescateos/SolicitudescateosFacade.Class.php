...
if (isset($_SESSION["cveUsuarioSistema"]) && $_SESSION["cveUsuarioSistema"] !== "") {
    ...
    class SolicitudescateosFacade {
        ...
        public function selectSolicitudescateos($SolicitudescateosDto) {
            ...
            $SolicitudescateosDto = $SolicitudescateosController->selectSolicitudescateos($SolicitudescateosDto);
...
//        $SolicitudescateosDto = $this->validarSolicitudescateos($SolicitudescateosDto);
            $SolicitudescateosController = new SolicitudescateosController();
            return $SolicitudescateosDto = $SolicitudescateosController->insertSolicitudescateos($param);
            /*
              if ($SolicitudescateosDto != "") {
              $dtoToJson = new DtoToJson($SolicitudescateosDto);
        ...
        public function updateSolicitudescateos($SolicitudescateosDto) {
            $SolicitudescateosDto = $this->validarSolicitudescateos($SolicitudescateosDto);
            $SolicitudescateosController = new SolicitudescateosController();
            $SolicitudescateosDto = $SolicitudescateosController->updateSolicitudescateos($SolicitudescateosDto);
            if ($SolicitudescateosDto != "") {
                $dtoToJson = new DtoToJson($SolicitudescateosDto);
        ...
        public function deleteSolicitudescateos($SolicitudescateosDto) {
            $SolicitudescateosDto = $this->validarSolicitudescateos($SolicitudescateosDto);
            $SolicitudescateosController = new SolicitudescateosController();
            $SolicitudescateosDto = $SolicitudescateosController->deleteSolicitudescateos($SolicitudescateosDto);
        ...
        public function consultaCateos($idJuez, $param) {
            if ($idJuez != "" && $idJuez != 0) {
                ...
                if ($juzgadoresDto != "" && count($juzgadoresDto) > 0) {
                    ...
                    $SolicitudescateosController = new SolicitudescateosController();
                    $resultados = $SolicitudescateosController->consultaCateos($juzgadoresDto->getIdJuzgador(), $param);
                    if ($resultados["type"] == "OK") {
                        ...
                        foreach ($resultados["data"] as $resultado) {
                            ...
                            $json .= '"nip":"' . utf8_encode($resultado["solicitudCateo"]->getNip()) . '",';
        ...
        public function consultaCateoDetalle($idCateo, $juez = "") {
            if ($idCateo != "" && $idCateo != 0) {
                $SolicitudescateosController = new SolicitudescateosController();
                $resultados = $SolicitudescateosController->consultaCateosDetalle($idCateo);
                if ($resultados["type"] == "OK") {
                    $juzgadosDao = new JuzgadosDAO();
                    $estatussolicitudesDao = new EstatussolicitudesDAO();
                    $estatussolicitudesDto = new EstatussolicitudesDTO();
...
//                    readfile("./../../../solicitudespdf/" . $resultado["file"]);
        ...
        public function cancelarSolicitudCateo($idSolicitudCateo, $motivoCancelacion) {
            if ($idSolicitudCateo != "" && $idSolicitudCateo > 0) {
                $SolicitudescateosController = new SolicitudescateosController();
                $SolicitudescateosDto = $SolicitudescateosController->cancelarSolicitudCateo($idSolicitudCateo, $motivoCancelacion);
//        if ($SolicitudescateosDto == true) {
//            $jsonDto = new Encode_JSON();
//            return $jsonDto->encode(array("totalCount" => "0", "text" => "REGISTRO ELIMINADO DE FORMA CORRECTA"));
//        }
        ...
        public function selectJuzgadoresGenericoJuzgado() {
            ...
            $juzgadoresDTO = new JuzgadoresDTO();
            $juzgado = $_SESSION["cveAdscripcion"];
            $juzgadoresDTO->setCveJuzgado("10322");
        ...
        public function getInfoWS($idJuzgado) {
            $solicitudCtr = new SolicitudescateosController();
        ...
        public function consultaImputadosCateos($param) {
            if (isset($param["imputados"]) || $param["imputados"] != "") {
                $consultaCtr = new SolicitudescateosController();
                $resultado = $consultaCtr->consultaImputadosCateos($param);
                return $resultado;
            } else {
                return array("type" => "Error");
            }
        ...
        public function cambioStatus($idSolicitudCateo, $cambiarStatus, $motivo) {
            if (($idSolicitudCateo != '') && ($cambiarStatus > 0) && ($motivo != '')) {
                $consultaCtr = new SolicitudescateosController();
                $resultado = $consultaCtr->cambioStatus($idSolicitudCateo, $cambiarStatus, $motivo);
                return $resultado;
            }
            return array("type" => 'Error');
        }
...
#OTROS DATOS
    ...
    $solicitudescateosFacade = new SolicitudescateosFacade();
...
// --> Accion para obtener la informacion a travez del Web Service
            ...
            echo $resultado;
        } else if (($accion == "consultaImputadosCateos")) {
            $param["imputados"] = $_POST["imputados"];
            $param["calle"] = $_POST["calle"];
            $param["municipio"] = $_POST["miunicipio"];
            $param["paginacion"] = true;
            $datosBusqueda = $solicitudescateosFacade->consultaImputadosCateos($param);
            echo json_encode($datosBusqueda, true);
        } else if (($accion == 'cambioStatus')) {
            $cambiarStatus = $_POST['cambiarStatus'];
            $motivo = $_POST['motivo'];
            $datosBusqueda = $solicitudescateosFacade->cambioStatus($idSolicitudCateo, $cambiarStatus, $motivo);
            echo json_encode($datosBusqueda, true);
        }
    } else {
