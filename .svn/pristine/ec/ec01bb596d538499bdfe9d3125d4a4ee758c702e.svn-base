...
class ReporteTocasEstatusController {
...
//                break;
   ...
   public function getAmparosPorToca($idCarpetajudicial,$proveedor) {
//      var_dump($idCarpetajudicial);
	...
	tblestatusamparos.desEstatus  ";
      $params["tables"] = " htsj_sigejupe.tblamparos tblamparos 
		INNER JOIN htsj_sigejupe.tblestatusamparos tblestatusamparos 
		ON tblamparos.cveEstatusAmparo = tblestatusamparos. 
		cveEstatusAmparo   ";
      $params["conditions"] = " (tblamparos.idCarpetaJudicial = " . $idCarpetajudicial . " ) AND
	(tblamparos.activo = 'S') ";
...
//      var_dump($rs);
   ...
   public function reporteTocas($tocasDto, $datos, $paginacion) {
      ...
      $params["fields"] .= "cj.idCarpetaJudicial,cj.numero,cj.anio,cj.fechaTermino,cj.fechaTermino, ec.cveEstatusCarpeta,ec.desEstatusCarpeta, tc.cveTipoCarpeta,tc.desTipoCarpeta,  tj.cveTipoJuzgado,tj.desTipoJuzgado, j.cveJuzgado,j.desJuzgado,st.cveTipoSentencia";
      $params["tables"] = "tblcarpetasjudiciales cj  INNER JOIN tbltiposcarpetas tc ON cj.cveTipoCarpeta = tc.cveTipoCarpeta INNER JOIN tblestatuscarpetas ec ON cj.cveEstatusCarpeta=ec.cveEstatusCarpeta LEFT JOIN tblsentenciastocas st ON st.idToca=cj.idCarpetaJudicial and ( st.activo = 'S') INNER JOIN tbljuzgados j ON cj.cveJuzgado = j.cveJuzgado INNER JOIN tbltiposjuzgados tj ON j.cveTipojuzgado = tj.cveTipoJuzgado ";
      $params["conditions"] = "cj.cveTipoCarpeta=6  AND cj.activo='S' AND tc.activo='S'  AND ec.activo='S' AND j.activo='S' AND tj.activo='S' ";
      if ($datos["numero"] != "") {
      ...
      if ($datos["mes"] != 0) {
         $params["conditions"].= " AND (month(cj.fechaTermino)=" . $datos['mes'] . ")";
         if ($datos["anioMes"] != "") {
            $params["conditions"].= " AND (year(cj.fechaTermino)=" . $datos['anioMes'] . ")";
         }
      }
      if ($datos["fechaInicial"] != "") {
         $fechaInicio = explode("/", $datos["fechaInicial"]);
         $params["conditions"].= " AND cj.fechaTermino >='" . $fechaInicio[2] . "-" . $fechaInicio[1] . "-" . $fechaInicio[0] . " 00:00:00' ";
         if ($datos["fechaFinal"] != "") {
            $fechaFinal = explode("/", $datos["fechaFinal"]);
            $params["conditions"].=" AND  cj.fechaTermino <= '" . $fechaFinal[2] . "-" . $fechaFinal[1] . "-" . $fechaFinal[0] . " 23:59:59 ' ";
         }
      }
      if ($datos["cveJuzgado"] != "") {
      ...
      $tocasArray = array();
      if ($rs->totalCount > 0) {
         $tocasArrayReturn = array();
         foreach ($rs->resultados as $key => $value) {
            foreach ($value as $key2 => $value2) {
               $tocasArray[$key][$key2] = $value2;
               if ($key2 == "idCarpetaJudicial")
                  $tocasArray[$key]["amparos"] = $this->getAmparosPorToca($value2,$this->proveedor);
            }
         }
         $tocasArrayReturn["Estatus"] = $rs->Estatus;
         $tocasArrayReturn["totalCount"] = $rs->totalCount;
         $tocasArrayReturn["mnj"] = $rs->mnj;
         $tocasArrayReturn["resultados"] = $tocasArray;
      }else{
         return json_encode($rs);
      }

      return json_encode($tocasArrayReturn);
   }
