...
class CarpetasjudicialesService {
    ...
    public function generarCarpetaDesdeSolicitud($idSolicitudAudiencia, $proveedor = null, $reclusosImputados = '') {
        ...
        try {
             ...
             */
            $logger->w_onError($beginLog . "COMIENZA PROGRAMA PARA CREAR CARPETAS JUDICIALES" . $endLog);
            ...
            $logger->w_onError("-->OBTENEMOS LA FECHA " . $fechaActual);
            ...
            if ($getSolicitud == '') {
                $logger->w_onError('no se encontro una solicitud de audiencia con el id = ' . $idSolicitudAudiencia);
                throw new Exception('no se encontro una solicitud de audiencia con el id = ' . $idSolicitudAudiencia);
            }

            $logger->w_onError("DATOS: " . json_encode($getSolicitud[0]->toString()));
            ...
            if ($getCatAudiencias == '') {
                $logger->w_onError('no se encontro la categoria de audiencia con id: ' . $cveCatAudienciaSolicitud . ', para obtener la etapa procesal');
                ...
                $logger->w_onError("AUDIENCIA: " . json_encode($catAudiencia->toString()));
            }
            ...
            $logger->w_onError("ETAPA PROCESAL: " . $cveEtapaProcesal);
            ...
            if ($getSolicitud[0]->getIdReferencia() == 0 || is_null($getSolicitud[0]->getIdReferencia())) {
                 ...
                 */
                $copiarImputados = $this->CopiarImputados($getSolicitud, $idCarpetaJudicial, $cveEtapaProcesal, $this->proveedor);
...
//                    throw new Exception($insertAntecedesCarpetas['mensaje']);
                ...
                $logger->w_onError("idReferencia: " . $idReferencia);
                /*
                ...
                $carpetaJudicialDto->setActivo('S');
                $logger->w_onError("CONSULTAMOS LA REFERENCIA");
                ...
                if ($getCarpetaJudicialById != '') {
                    ...
                    $logger->w_onError("DATOS: " . json_encode($getCarpetaJudicialById[0]->tostring()));
                    ...
                    $logger->w_onError("ultimaEtapaProcesal: " . $ultimaEtapaProcesal);
                    $logger->w_onError("ultimaTipoCarpeta: " . $ultimoTipoCarpeta);
                    /*
...
//                        throw new Exception('no se puede regresar a una etapa procesal anterior de ' . $ultimaEtapaProcesal . ' a  ' . $cveEtapaProcesal . ' ');
//                    }
                } else {
...
//si requiere causa creamos una nueva carpeta de tipo carpeta = control (2)
                        if ($catAudiencia->getCausa() == 'S') {
                            $logger->w_onError("Se tendra que generar una carpeta judicial de tipo 2");
                            $cveTipoCarpeta = 2;
                             ...
                             * obtenemos el cveJuzgado
                             */
                            $logger->w_onError("Busco el juzgado");
                            $getJuzgado = $this->getJuzgado($getSolicitud[0]->getCveJuzgado(), 1, $this->proveedor);
                            if ($getJuzgado['estatus'] == 'error') {
                                throw new Exception($getJuzgado['mensaje']);
                            }
                            $logger->w_onError("cveJuzgado: " . $getJuzgado['data']);
                            $cveJuzgado = $getJuzgado['data'];
                            ...
                            if ($cveTipoCarpeta === 6) {
                                ...
                                $logger->w_onError("Obtenemos el numero de contador");
                                $contadorDto = new ContadoresDTO();
                                $contadorDto->setCveTipoCarpeta($cveTipoCarpeta);
                                $contadorDto->setAnio(date('Y')); //Cambiar para a�o de bd
                                $contadorDto->setCveJuzgado($cveJuzgado);
                                $contadorController = new ContadoresController();
                                ...
                                $getSolicitud[0]->setNumero($getContador[0]->getNumero());
                                $getSolicitud[0]->setAnio($getContador[0]->getAnio());
                                $logger->w_onError("Numero:" . $getContador[0]->getNumero());
                                $logger->w_onError("anio:" . $getContador[0]->getAnio());
                            }

                            /*
                             * creamos la carpeta judicial
                             */
                            $logger->w_onError("Creamos la carpeta judicial");
                            $crearCarpetaJudicial = $this->crearCarpetaJudicial($getSolicitud, $cveEtapaProcesal, $cveTipoCarpeta, $cveConclucion, $ponderacion, $cveJuzgado, $this->proveedor);
                            if ($crearCarpetaJudicial['estatus'] == 'error') {
                                throw new Exception($crearCarpetaJudicial['mensaje']);
                            }
                            ...
                            $idCarpetaJudicial = $crearCarpetaJudicial['data']->getIdCarpetaJudicial();
                            $logger->w_onError("Obtenemos el nuevo idCarpetaJudicial: " . $idCarpetaJudicial);
                            /*
                             * si la carpeta se creó correctamente se copian desde la solicitud
                             * imputados
                             * ofendidos
                             ...
                             * relaciones (violencia de genero en caso de que exista)
                             */
                            $logger->w_onError("Copiamos imputados");
                            $copiarImputados = $this->CopiarImputados($getSolicitud, $idCarpetaJudicial, $cveEtapaProcesal, $this->proveedor);
                            $logger->w_onError("Copiamos Ofendidos");
                            $copiarOfendidos = $this->CopiarOfendidos($idSolicitudAudiencia, $idCarpetaJudicial, $this->proveedor);
                            $logger->w_onError("Copiamos delitos");
                            $copiarDelitos = $this->CopiarDelitos($idSolicitudAudiencia, $idCarpetaJudicial, $this->proveedor);
                            $logger->w_onError("Copiamos Relaciones");
                            $copiarRelacion = $this->CopiarRelacion($idSolicitudAudiencia, $idCarpetaJudicial, $this->proveedor);
                            /*
                             * Falta violencia de genero aqui
                             */
                            ...
                            $logger->w_onError("Consulta los datos de la carpeta padre");
                            $getCarpetaJudicial = $carpetaJudicialDao->selectCarpetasjudiciales($carpetaJudicialDto, '', $this->proveedor);

                            if ($getCarpetaJudicial != '') {
                                if (((int) $cveEtapaProcesal == 1) || ((int) $cveEtapaProcesal == 2)) {
//                                    $getSolicitud
                                    $carpetaJudicialDto->setFechaTermino($fechaActual);
                                    $carpetaJudicialDto->setCveConclucion(8); //OTRO
                                    $carpetaJudicialDto->setCveEstatusCarpeta(2); //OTRO
                                }
                                ...
                                if ($updateCarpeta == '') {
                                    ...
                                    $logger->w_onError("Datos: " . json_encode($updateCarpeta[0]->tostring()));
                                }
                            } else {
                                $logger->w_onError("La carpeta padre no existe");
                                throw new Exception('no se pudo modificar la fecha de radicación de la carpeta anterior con id : ' . $idReferencia);
                            }
                             ...
                             */
                            $logger->w_onError("Insertamos antecedente");
                             ...
                             * editamos la solicitud de audiencia idReferencia, numero, anio, juzgado
                             */
                            $logger->w_onError("Actualizamos los datos de la solicitud");
                            $editarSolicitudAudiencia = $this->updateSolicitudAudiencia($getSolicitud, $crearCarpetaJudicial['data'], $this->proveedor);
                            if ($editarSolicitudAudiencia['estatus'] == 'error') {
                                throw new Exception($editarSolicitudAudiencia['mensaje']);
                            }
                             ...
                             */
                            $logger->w_onError("Modificamos la etapa procesal");
                            $this->modificarEtapaProcesalImputadosCerrarCarpeta($cveEtapaProcesal, $getSolicitud, $this->proveedor);
                    ...
                    } else if ($ultimoTipoCarpeta == 2) { //si es tipo de carpeta = 2 (de control) regresamos carpeta igual
                        $estatus = 'igual';
                        $mensaje = 'la etapa procesal es igual a la última y ya se generó una carpeta de control';
                        $data = $getSolicitud;
                    } else if ($ultimoTipoCarpeta == 3) {
                        $estatus = 'igual';
                        $mensaje = 'La etapa procesal corresponde a juicio y no se genera una carpeta judicial';
                        $data = $getSolicitud;
                    } else if ($ultimoTipoCarpeta == 4) {
                        $estatus = 'igual';
                        $mensaje = 'La etapa procesal corresponde a juicio de tribunal y no se genera una carpeta judicial';
                        $data = $getSolicitud;
                    } else if ($ultimoTipoCarpeta == 5) {
                        $estatus = 'igual';
                        $mensaje = 'La etapa procesal corresponde a expediente y no se genera carpeta judicial';
                        $data = $getSolicitud;
                    } else if ($ultimoTipoCarpeta == 6) {
                        $estatus = 'igual';
                        $mensaje = 'La etapa procesal corresponde a toca y no se genera carpeta judicial';
                        $data = $getSolicitud;
                    }
...
//si no se crea una nueva carpeta judicial con la etapa procesal 3
                        ...
                        $logger->w_onError("Se tendra que generar una carpeta judicial de tipo: " . $cveTipoCarpeta);
...
//                                throw new Exception('no se pudo modificar la fecha de radicación de la carpeta anterior con id : ' . $idReferencia);
                         ...
                         * intentamos cerrar carpeta si todos sus imputados se encuentran en una etapa posterior
                         */
                        $logger->w_onError("Verificar si la carpeta origen debe terminarse ");
                        $this->modificarEtapaProcesalImputadosCerrarCarpeta($cveEtapaProcesal, $getSolicitud, $this->proveedor);
...
//$this->modificarEtapaProcesalImputadosCerrarCarpeta($cveEtapaProcesal, $getSolicitud[0]->getIdSolicitudAudiencia(), $this->proveedor);
            ...
            if (is_null($proveedor)) {
                $this->proveedor->execute('COMMIT'); //COMMIT
            }
...
//$carpetaJudicialDto->setCveConclucion($cveConclucion);
            ...
            if ($proveedor == null) {
                $proveedor->execute('COMMIT'); //COMMIT
            }
     ...
     * @return array
     */
    public function CopiarImputados($solicitudAudiencia, $idCarpetaJudicial, $cveEtapaProcesal, $proveedor = null, $byIdImputado = null) {
        $beginLog = "/**************************/ ";
        $endLog = " /**************************/";
        $logger = new Logger("/../../logs/", "CarpetasJudicialesService");
        ...
        try {
            ...
            $imputadoSolicitudDto->setIdSolicitudAudiencia($solicitudAudiencia[0]->getIdSolicitudAudiencia());
...
//obtenemos el id del imputado que se guardo
                    $idImputadoCarpetaPadre = $imputadoSolicitud->getIdImputadoCarpeta();
                    $idImputadoCarpeta = $insertImputadoCarpeta[0]->getIdImputadoCarpeta();

                    if (((int) $cveEtapaProcesal == 1) || ((int) $cveEtapaProcesal == 2)) {
//$imputadosCarpetaPadre 
//NO HAGO NADA PORQUE YA SE CONCLUYO LA CARPETA CON ANTERIORIDAD
                        $idReferencia = $solicitudAudiencia[0]->getIdReferencia();
                        $logger->w_onError("idReferencia: " . $idReferencia);
                    } elseif ((int) $cveEtapaProcesal == 3) {
                        $tmpImputadoCarpetaDto = new ImputadoscarpetasDTO();
                        $tmpImputadoCarpetaDto->setIdImputadoCarpeta($idImputadoCarpetaPadre);
                        $tmpImputadoCarpetaDto->setCveEtapaProcesal($cveEtapaProcesal);
                        $tmpImputadoCarpetaDao = new ImputadoscarpetasDAO();
                        $tmpImputadoCarpetaDto = $tmpImputadoCarpetaDao->updateImputadoscarpetas($tmpImputadoCarpetaDto, $proveedor);

                        if ($tmpImputadoCarpetaDto == "") {
                            throw new Exception('No se logro actualizar la etapa procesal del imputado en la carpeta padre');
                        } else {
                            /*
                             * Preguntar a rito cual es la tabla para guardar las conclusiones
                             */
                        }
                    }
                     ...
                     */
                    $imputadosCarpetaPadre[$count] = $imputadoSolicitud->getIdImputadoCarpeta();
                    $count++;

                    $_SESSION['idImputadoCarpetaAuto'][] = $imputadoSolicitud->getIdImputadoCarpeta();
                    $logger->w_onError("idImputadoCarpeta: " . $imputadoSolicitud->getIdImputadoCarpeta());
                    /*
                    ...
                    $logger->w_onError("IMPUTADOS" . json_encode($_SESSION['imputados_copia']));
...
//NO HAGO NADA PORQUE YA SE CONCLUYO LA CARPETA CON ANTERIORIDAD
                    ...
                    $tmpImputadoCarpetaDto = new ImputadoscarpetasDTO();
                    $tmpImputadoCarpetaDto->setIdCarpetaJudicial($idReferencia);
                    $tmpImputadoCarpetaDto->setActivo("S");

                    $tmpImputadoCarpetaDao = new ImputadoscarpetasDAO();
                    $tmpImputadoCarpetaDto = $tmpImputadoCarpetaDao->selectImputadoscarpetas($tmpImputadoCarpetaDto, " And cveEtapaProcesal<3 ", $proveedor);

                    if ($tmpImputadoCarpetaDto == "") {
                        $tmpCarpetaDto = new CarpetasjudicialesDTO();
                        $tmpCarpetaDto->setIdCarpetaJudicial($idReferencia);
                        $tmpCarpetaDto->setFechaTermino($fechaActual); //Actualiza kary
                        $tmpCarpetaDto->setCveEstatusCarpeta(2);
                        $tmpCarpetaDto->setIdCarpetaJudicial($idReferencia);
                        $tmpCarpetaDao = new CarpetasjudicialesDAO();
                        $tmpCarpetaDto = $tmpCarpetaDao->updateCarpetasjudiciales($tmpCarpetaDto, $proveedor);
                        if ($tmpCarpetaDto == "") {
                            throw new Exception('no se logro concluir la carpeta padre');
                        }
                    }
                }
            ...
            if ($tmp == null) {
                $proveedor->excecute('COMMIT'); //COMMIT
            }

            $response = [
                'estatus' => 'ok',
                'mensaje' => 'los imputados de la solicitud de audiencia con id . ' . $solicitudAudiencia[0]->getIdSolicitudAudiencia() . ' fueron copiados a carpetas judiciales con la carpeta id ' . $idCarpetaJudicial
            ];
        } catch (Exception $e) {
...
//echo "<hr><br><br>";
    ...
    public function getPonderacion($idSolicitudAudiencia, $proveedor = null, $byIdImputadoSolicitud = null) {
        ...
        foreach ($getImpofedelSolicitud as $impofedelSolicitud) {
            ...
            if ($getDelitoSolicitud[0]->getCveDelito() != 118) {
...
//si el num de imputados de la nueva son igual a los de referencia se cierra la carpeta de referencia
     ...
     */
    public function modificarEtapaProcesalImputadosCerrarCarpeta($nuevaEtapaProcesal, $solicitudAudienciaDto, $proveedor = null) {
        $logger = new Logger("/../../logs/", "CarpetasJudicialesService");
        $imputadosSolicitudDto = new ImputadossolicitudesDTO();
        $imputadosSolicitudDao = new ImputadossolicitudesDAO();
        $idCarpetaJudicial = 0;
        $imputadosSolicitudDto->setIdSolicitudAudiencia($solicitudAudienciaDto[0]->getIdSolicitudAudiencia());
        $imputadosSolicitudDto->setActivo('S');
        $logger->w_onError("***********CONSULTAMOS A LOS IMPUTADOS ACTIVOS DE LA SOLICITUD DE AUDIENCIA");
...
//editar cada imputadoCarpeta con la nueva etapa procesal
            $logger->w_onError("***********ACTUALIZAR LA ETAPA PROCESAL DEL IMPUTADO: " . $imputadoSolicitud->getIdImputadoCarpeta());
        ...
        $logger->w_onError("***********TIPO DE CARPETA: " . $tipoCarpeta);
        if ((int) $tipoCarpeta == 3) {
            $logger->w_onError("***********SI EL TIPO DE CARPETA ES 3, CONSULTAR EL REGISTRO DE ANTECEDES PARA OBTENER LA CARPETA PADRE");
            $antecedesCarpetasDto = new AntecedescarpetasDTO();
            $antecedesCarpetasDao = new AntecedescarpetasDAO();
            $antecedesCarpetasDto->setIdCarpetaHija($getCarpeta[0]->getIdCarpetaJudicial());
            $antecedesCarpetasDto->setActivo('S');
            $antecedesCarpetasDto = $antecedesCarpetasDao->selectAntecedescarpetas($antecedesCarpetasDto, "", $proveedor);
            if ($antecedesCarpetasDto == '')
                throw new Exception('No fue posible encontrar el registro de antecedente para la carpeta judicial');
            $logger->w_onError("***********SE OBTUVO EL REGISTRO DE ANTECEDES, OBTENER DATOS DE LA CARPETA PADRE");
            $carpetaJudicialTmpDto = new CarpetasjudicialesDTO();
            $carpetaJudicialTmpDto->setIdCarpetaJudicial($antecedesCarpetasDto[0]->getIdCarpetaPadre());
            $carpetaJudicialTmpDto = $carpetaJudicialDao->selectCarpetasjudiciales($carpetaJudicialTmpDto, "", $proveedor);
            if ($carpetaJudicialTmpDto == '') {
                throw new Exception('No fue posible obtener los datos de la carpeta de antecedente');
            } else {
                $idCarpetaJudicial = $carpetaJudicialTmpDto[0]->getIdCarpetaJudicial();
                $etapaProcesalCarpeta = $carpetaJudicialTmpDto[0]->getCveEtapaProcesal();
            }
            $imputadosCarpetasDto->setCveEtapaProcesal('');
        } else {
            $idCarpetaJudicial = $getCarpeta[0]->getIdCarpetaJudicial();
            $etapaProcesalCarpeta = $getCarpeta[0]->getCveEtapaProcesal();
        }
        $logger->w_onError("***********ID DE LA CARPETA: " . $idCarpetaJudicial);
        $logger->w_onError("***********ETAPA PROCESAL DE LA CARPETA: " . $etapaProcesalCarpeta);
        ...
        $imputadosCarpetasDto->setActivo('S');
        $logger->w_onError("***********CONSULTAR A LOS IMPUTADOS ACTIVOS DE LA CARPETA: " . $idCarpetaJudicial);
        ...
        foreach ($getImputadosCarpetas as $imputadoCarpeta) {
            $logger->w_onError("***********ETAPA PROCESAL DEL IMPUTADO " . $imputadoCarpeta->getIdImputadoCarpeta() . " : " . $imputadoCarpeta->getCveEtapaProcesal());
            if ($tipoCarpeta == 1) {
                $countCerrarCarpeta ++;
            } else {
        ...
        $logger->w_onError("***********NUMERO TOTAL DE IMPUTADOS ACTIVOS SIN SOBRESEIMIENTO: " . $countImputadosCarpeta);
        $logger->w_onError("***********IMPUTADOS EN LA ETAPA PROCESAL DE LA CARPETA: " . $countCerrarCarpeta);
//si todos los imputados ya estan en una etapa superior se cierra la carpeta
        if ($countImputadosCarpeta == $countCerrarCarpeta) {
            $logger->w_onError("***********SE TERMINA LA CARPETA DEBIDO A QUE TODOS LOS IMPUTADOS SE ENCUENTRAN EN UNA ETAPA PROCESAL DIFERENTE O TIENEN SOBRESEIMIENTO, ID DE CARPETA: " . $idCarpetaJudicial);
...
$response = $carpetasjudicialesController->generarCarpetaDesdeSolicitud(1109120);
print_r($response);
*/