...
class SoapClient
{
    ...
    {
        if (!$soapMessage) {
            throw new \sigejupev2\webservice\cliente\zimbra\ZCS\Exception(curl_error($this->curlHandle), curl_errno($this->curlHandle));
        }
        ...
        if ($fault) {
            throw new \sigejupev2\webservice\cliente\zimbra\ZCS\Exception($fault->Detail->children()->Error->Code);
        }
