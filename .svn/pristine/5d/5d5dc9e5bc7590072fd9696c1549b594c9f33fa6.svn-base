    ...
    public function insertDefensoresimputadossolicitudes($defensoresimputadossolicitudesDto, $proveedor = null, $bitacora = true) {
        ...
        if ((!$error)) {
            ...
            if ($defensoresimputadossolicitudesDto != "") {
                ...
                if ($defensoresimputadossolicitudesDto[0]->getCveTipoDefensor() == 6 || $defensoresimputadossolicitudesDto[0]->getCveTipoDefensor() == 1) {
                    $fecha = date("d/m/Y");
                    $hora = date("H:i:s");


                    if ($rsImputados[0]->getCveTipoPersona() == 1) {
                        $nombre = $rsImputados[0]->getNombre();
                        $paterno = $rsImputados[0]->getPaterno();
                        $materno = $rsImputados[0]->getMaterno();
                    } else {
                        $nombre = $rsImputados[0]->getNombreMoral();
                        $paterno = "";
                        $materno = "";
                    }

                    if ($rsSolicitudAudiencia[0]->getCveUsuario() == "7137") {
                        $resultWebSerice = "idReferencia=" . $defensoresimputadossolicitudesDto[0]->getIdDefensorImputadoSolicitud() .
                                "&nombreRemite=" . "pj" .
                                "&procedencia=" . "PJE" .
                                "&cargo=" . 28 .
...
//                            
                    if (json_last_error() == JSON_ERROR_NONE) {
                        if ($respuestaWebService != "{error:Los parametros no son correctos}") {
                            ...
                            $defensoresimputadossolicitudesAuxDto->setIdDefensorImputadoSolicitud($defensoresimputadossolicitudesDto[0]->getIdDefensorImputadoSolicitud());
                            $defensoresimputadossolicitudesAuxDto->setTelefono(utf8_decode($respuesta));
                            $rsDefensor = $defensoresimputadossolicitudesDao->updateDefensoresimputadossolicitudesRSP($defensoresimputadossolicitudesAuxDto, $proveedor);
                        }
                    } else {
        ...
        if ((!$error)) {
            ...
            if ($defensoresimputadossolicitudesDto != "") {
                ...
                if ($defensoresimputadossolicitudesDto[0]->getCveTipoDefensor() == 6 || $defensoresimputadossolicitudesDto[0]->getCveTipoDefensor() == 1) {
                    ...
                    if ($rsSolicitudAudiencia[0]->getCveUsuario() == "7137") {
                        $resultWebSerice = "idDefensor=" . $defensoresimputadossolicitudesDto[0]->getIdDefensorImputadoSolicitud() .
                                "&nombreRemite=" . "pj" .
                                "&procedencia=" . "PGJ" .
                                "&cargo=" . 28 .
                                "&fechaSolicitud=" . $fecha .
                    ...
                    if (json_last_error() == JSON_ERROR_NONE) {
                        ...
                        $defensoresimputadossolicitudesAuxDto->setIdDefensorImputadoSolicitud($defensoresimputadossolicitudesDto[0]->getIdDefensorImputadoSolicitud());
                        $defensoresimputadossolicitudesAuxDto->setTelefono(utf8_decode($respuesta));
                        $rsDefensor = $defensoresimputadossolicitudesDao->updateDefensoresimputadossolicitudesRSP($defensoresimputadossolicitudesAuxDto);
                        if ($rsDefensor != "") {
