        ...
        if(!$error) {
            ...
            //if($programacionCateosTmp == "") {
                ...
                if($programacionCateosDto != "") {
                    ...
                    $bitacoramovimientosDto = new BitacoramovimientosDTO();
                    $cveAccion = 340;
                    $descripcion = "UPDATE tabla tblprogramacioncateos idProgramacionCateo: " . $programacionCateosDto[0]->getIdProgramacionCateo();
                    $bitacoramovimientosDto->setCveAccion($cveAccion);
...
//            }
    ...
    public function juezCateo($juzgadoDto, $programacionCateosDto, $proveedor = null){
        ...
        /*
         * Consultar el grupo al que pertenece el juzgado enviado como parámetro
         */
        $gruposJuzgadosDto = new GruposjuzgadosDTO();
        $gruposJuzgadosDao = new GruposjuzgadosDAO();
        $gruposJuzgadosDto->setCveJuzgado($juzgadoDto->getCveJuzgado());
        ...
        if ( $gruposJuzgadosDto != "" ) {
            foreach ( $gruposJuzgadosDto as $grupoJuzgado ) {
                 ...
                 * entre la fecha y hora enviadas como parámetro para saber que
                 * juez está disponible para realizar el cateo
                 */
                $programacionCateosDto = new ProgramacioncateosDTO();
                $programacionCateosDao = new ProgramacioncateosDAO();
                //$programacionCateosDto->setCveGrupoJuzgado($grupoJuzgado->getCveGrupoJuzgado());
                ...
                $order = " AND CAST('" . $fecha . "' AS DATETIME) >= fechaInicio
                           AND CAST('" . $fecha . "' AS DATETIME) <= fechaFinal AND cveGrupoJuzgado IN (" . $cadenaJuzgados . ")";
                $programacionCateosDto = $programacionCateosDao->selectProgramacioncateos($programacionCateosDto, $order, $this->proveedor);
                if($programacionCateosDto!= "") {
                    /*
                     * Se encontraron resultados, verificar si existe más de un juzgador disponible
                     * en la fecha y hora enviadas como parámetro, en caso de haber más de un juzgador
                     * revisar cuántos cateos tiene asignados, retornar el id del juzgador que tenga
                     * menos cateos
                     */
...
//                        echo "</pre>";
                    ...
                    if ( $juzgadoresDto != "" ) {
                        ...
                        $error = true;
                        $msg = " Ocurrrió un error al consultar los datos del juez";
                    }
                } else {
                    $error = true;
                    $msg = " No se encontraron jueces programados para atender cateos para el juzgado " . utf8_encode($juzgadoTmp[0]->getDesJuzgado()) . " en el día " . $fechaInicio[0] . " a las " . $fechaInicio[1];
                }
            }
        } else {
            $error = true;
            $msg = " No se encontraron datos de programación de cateos para el juzgado " . utf8_encode($juzgadoTmp[0]->getDesJuzgado());
        }
        if ( !$error ) {
            $datos = array(
                "estatus"    => "ok",
    ...
    public function simuladorCateos($juzgadosDto, $programacioncateosDto, $proveedor = null){
        ...
        /*
         * Ciclo para definir los días en que se mostrarán los datos para el simulador
         */
        ...
        for( $n = 0; $n < count($dias); $n++ ) {
            /*
             * Para cada uno de los días verificar que juez puede atender algun catoe
             */
    ...
    public function eliminarSeleccionados($programacioncateosDto, $param, $proveedor = null) {
            ...
            for ( $n = 0; $n < count($datos); $n++ ) {
                if ( (int)$datos[$n]->idProgramacionCateo != 0 ) {
                    ...
                    if($programacionCateosDto != "") {
                        ...
                        $error = true;
                        $msg[] = array(utf8_encode('Ocurrió un error al eliminar el registro!'));
                    }
                } else {
                    $error = true;
                    $msg[] = array(utf8_encode('Ocurrió un error al recibir los datos a eliminar!'));
                }
                if ( $error ) {
                    break;
                }
        ...
        if ( !$error ) {
            ...
            if ( $insertarBitacora == "" ) {
                $error = true;
                $msg[] = array(utf8_encode('Ocurrió un error al registrar la acción realizada!'));
            } else {
                $error = false;
            }
        }
...
$programacionCateosDto = new ProgramacioncateosDTO();
$programacionCateosDto->setFechaInicio("2016-02-17 06:59:59");
