...
if (isset($_SESSION["cveUsuarioSistema"]) && $_SESSION["cveUsuarioSistema"] !== "") {
    ...
    <input type="hidden" id="hiddencveTipoAuto" value="">
    ...
    <div class="panel panel-default">
        <div class="panel-heading">
            <h5 class="panel-title" id="h5titulo">                                                            
                Reporte de Autos de Vinculaci&oacute;n
            </h5>
        </div>
        <div  id="panelPrincipal" class="panel-body">
            <div id="divFormulario" class="form-horizontal">
                <div id="divPanelControl">
                    ...
                    <div class="form-group"> 
                        <label class="control-label col-md-4">Tipo de auto</label>
                        <div class="col-md-1">
                            <input type="checkbox" id="checkTipoAuto" class="form-inline" onclick="tipoAuto()">
                        </div>
                        <div class="col-md-3" id="divTipoAuto" style="display:none">
                            <select class="form-control" name="cmbTipoAuto" id="cmbTipoAuto">
                                <option value="0">Seleccione una opci&oacute;n</option>
                            </select>
                        </div>                                
                    </div>
    ...
    <script type="text/javascript">
        ...
        exportar = function (accion, div) {
            var usuario = '<?php echo $_SESSION["nombre"]; ?>';
            var nombreArchivo = "REPORTE_AUTOS";
        ...
        limpiar = function (bandera) {
            ...
            $("#hiddenVariables").val("");
            $("#h5titulo").html("Reporte de Autos de Vinculaci&oacute;n");
            $("#checkTipoAuto").prop("checked", "");
        ...
        filtro = function () {
            ...
            $("#idLabelDescripcion").text("Reporte Autos Vinculaci\u00F3n" + mensaje);
            if ($("#checkTipoAuto").is(":checked")) {
                if ($("#cmbTipoAuto").val() != 0) {
                    strDatos += "&cveTipoAuto=" + $("#cmbTipoAuto").val();
                }
            }
            return strDatos;
        };
        ...
        preGestorConsulta = function (nivel, json, i) {
            ...
            if (nivel == 2) {
                if (json.resultados[i].cveTipoAuto) {
                    strDatos = "&cveTipoAuto=" + json.resultados[i].cveTipoAuto;
                    $("#hiddencveTipoAuto").val(json.resultados[i].cveTipoAuto);
                }
            }
            if (nivel == 3) {
                if ($("#checkTipoAuto").is(":checked")) {
                    strDatos = "&cveTipoAuto=" + $("#hiddencveTipoAuto").val();
                }
        ...
        gestorNivel = function (json, bandera) {
            ...
            switch ($("#hiddenNivel").val()) {
                case "1":
                    ...
                    if ($("#checkTipoAuto").is(":checked")) {
                        table += "<th>Tipo Vinculaci&oacute;n</th>";
                    }
                    ...
                    if (json.totalCount > 0) {
                        ...
                        for (var i = 0; i < json.totalCount; i++) {
                            ...
                            if ($("#checkTipoAuto").is(":checked")) {
                                table += "<td>" + json.resultados[i].desTipoAuto + "</td>";
                            }
                ...
                case "4":
                    ...
                    break;
                case "5":
                    table += "<th>N&uacute;m.</th><th>Tipo de auto</th><th>Imputado</th>";
                    table += "<th>Carpeta</th><th>Fecha Registro</th><th>Codigo</th>";
                    table += "</tr></thead><tbody>";
                    for (var i = 0; i < json.totalCount; i++) {
                        table += "<tr>";
                        table += "<td> " + (i + 1 + indice) + "</td>";
                        ...
                        //var codigo = consultarAudienciasCodigo(json.resultados[i].numero,json.resultados[i].anio,json.resultados[i].cveJuzgado);
        ...
        consultar = function (bandera, nivel) {
            ...
            var strDatos = "accion=consultar_autos_Reporte";
            strDatos += "&activo=S";
            strDatos += "&paginacion=true";
            if ($("#checkTipoAuto").is(":checked")) {
                strDatos += "&checkTipoAuto=true";
            }
            ...
            $.ajax({
                type: "POST",
                url: "../fachadas/sigejupe/reportes/ReporteautosFacade.Class.php",
                data: strDatos,
                async: true,
                dataType: "html",
            ...
            switch ($("#hiddenNivel").val()) {
                case "1":
                    ...
                    for (var i = 0; i < json.totalCount; i++) {
                        ...
                        if ($("#checkTipoAuto").is(":checked")) {
                            categorias[i] = json.resultados[i].desTipoAuto;
                        } else {
                            categorias[i] = "AUTOS";
                        }
    ...
    //        }
        ...
        calcularPaginas = function () {
            $("#inputRegresar2").hide();
            var url = "../fachadas/sigejupe/reportes/ReporteautosFacade.Class.php";
            var strDatos = "accion=consultar_autos_Reporte";
        ...
        encabezado = function () {
            ...
            $("#h5titulo").html("<span style='text-decoration: underline; cursor:pointer;' onclick='regresar(0,false);'>Reporte de Autos de Vinculaci&oacute;n</span>" + mensaje);
        };
        ...
        cargarTipoAuto = function () {
            var strDatos = "accion=consultar";
            strDatos += "&activo=S";
            $.ajax({
                type: "POST",
                url: "../fachadas/sigejupe/tiposautos/TiposautosFacade.Class.php",
                data: strDatos,
                async: true,
                dataType: "html",
                beforeSend: function (objeto) {
                ...
                success: function (datos) {
                    ...
                    if (json.totalCount > 0) {
                        for (var i = 0; i < json.totalCount; i++) {
                            $("#cmbTipoAuto").append($('<option></option>').val(json.data[i].cveTipoAuto).html(json.data[i].desTipoAuto));
                        }
                    }
                },
                error: function (objeto, quepaso, otroobj) {
                    mostrarMensaje("Error en la peticion (cargar Tipos de autos):\n\n" + quepaso, 3);
                }
            });
        };
                                ...
                                $.each(response.perfiles[0].perfil[i].permisos, function (k, vnombre) {
                                    if (vnombre.nomFormulario == "REPORTES") {
                                        ...
                                        $.each(hijos, function (k2, nombreHijo) {
                                            if (nombreHijo.nomFormulario == 'AUTOS VINCULACION') {
