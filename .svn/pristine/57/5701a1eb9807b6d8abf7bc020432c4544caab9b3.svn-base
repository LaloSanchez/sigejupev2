 ...
 */
include_once(dirname(__FILE__) . "/../../../controladores/sigejupe/domiciliosofendidossolicitudes/DomiciliosofendidossolicitudesController.Class.php");
include_once(dirname(__FILE__) . "/../../../controladores/sigejupe/telefonosofendidossolicitudes/TelefonosofendidossolicitudesController.Class.php");
...
include_once(dirname(__FILE__) . "/../../../controladores/sigejupe/tutoresofendidos/TutoresofendidosController.Class.php");
...
///////////////////////////////////////////////////////////////////////////////
    ...
    public function validaEstatusSolicitud($idSolicitudAudiencia, $accion, $proveedor = null) {
        ...
        if ($getSolicitud == '') {
            return [
                'estatus' => 'error',
                'mensaje' => 'La solicitud de audiencia no existe.'
            ];
        }
...
//                }
        ...
        } elseif ($OfendidossolicitudesDto->getCveTipoPersona() == 2 || $OfendidossolicitudesDto->getCveTipoPersona() == 3 || $OfendidossolicitudesDto->getCveTipoPersona() == 4 || $OfendidossolicitudesDto->getCveTipoPersona() == 5) {
...
//                }
    ...
    public function agregarOfendido($OfendidossolicitudesDto, $proveedor = null, $usaBitacora = true) {
        ...
        $validarEstatusSolicitud = $this->validaEstatusSolicitud($OfendidossolicitudesDto->getIdSolicitudAudiencia(), 'agregar', $proveedor);
        ...
        } elseif ($ofendido->getCveTipoPersona() == 2 || $ofendido->getCveTipoPersona() == 3 || $ofendido->getCveTipoPersona() == 4 || $ofendido->getCveTipoPersona() == 5) {
...
//            $OfendidossolicitudesDto->setCveVictimaDeViolenciaDeGenero($arreglo["ofendido"]["cveVictimaDeViolendiaDeGenero"]);
                ...
                } else if (($OfendidossolicitudesDto->getCveTipoPersona() == 2) || ($OfendidossolicitudesDto->getCveTipoPersona() == 3) || ($OfendidossolicitudesDto->getCveTipoPersona() == 4) || ($OfendidossolicitudesDto->getCveTipoPersona() == 5)) {
...
//                }
    ...
    public function creaOfendido($data, $proveedor = null) {
        ...
        try {
            ...
            if ($data == '')
                throw new Exception('Ocurrio un error al parsear el json, por favor ponte en contacto con el administrador.');

            $idOfendidosArray = array();
            ...
            foreach ($data as $index => $arreglo) {
                ...
                if (!isset($arreglo['ofendido']['idSolicitudAudiencia']))
                    throw new Exception('El parametro idSolicitudAudiencia no esta presente, es obligatorio');

                if ($arreglo["ofendido"]["idSolicitudAudiencia"] == '')
                    throw new Exception('el parametro idSolicitudAudiencia es requerido para poder agregar ofendido(s)');
                ...
                if (isset($arreglo['ofendido']['defensores']) && count($arreglo['ofendido']['defensores'])) {
                    foreach ($arreglo['ofendido']['defensores'] as $indexDefensor => $defensor) {
                        ...
                        if ($creaDefensor['status'] == 'error') {
                            ...
                            $errorDefensor = "el error ocurrió en el ofendido con índice " . $index . ' y defensor con índice ' . $indexDefensor;
    ...
    public function eliminarOfendidoAudiencia($OfendidossolicitudesDto, $proveedor) {
        $this->proveedor = $proveedor;
        $ofendidoSolicitudDto = new OfendidossolicitudesDTO();
        $ofendidoSolicitudDao = new OfendidossolicitudesDAO();
        ...
        try {
            ...
            $bitacora = new BitacoramovimientosController();
            $bitacoraOfendido = $bitacora->agregar(360, 'ELIMINAR tblofendidossolicitudes y sus relaciones de violencia de genero, trata personas y efectos sexuales', 'txt', serialize($OfendidossolicitudesDto), '', $this->proveedor);
            if (!count($bitacoraOfendido))
                throw new Exception('no se pudo guardar la accion eliminar ofendido en bitacora');
