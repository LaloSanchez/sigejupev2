...
//                }
    ...
    public function creaOfendido($data, $proveedor = null)
    {
        ...
        try {
            ...
            if ($data == '') throw new Exception('Ocurrio un error al parsear el json, por favor ponte en contacto con el administrador.');
            ...
            foreach ($data as $index => $arreglo) {
                ...
                if (!isset($arreglo['ofendido']['idSolicitudAudiencia'])) throw new Exception('El parametro idSolicitudAudiencia no esta presente, es obligatorio');

                if ($arreglo["ofendido"]["idSolicitudAudiencia"] == '') throw new Exception('el parametro idSolicitudAudiencia es requerido para poder agregar ofendido(s)');
