...
class ExhortosElectronicosServer {

    public function __construct() {
        
    }

    public function selectExhorto($u, $p, $json) {
        $return = array();
        // if ($this->isJSON($json)) {
        ...
        if ($jsonSolicitudCateo != "") {
            if ($this->isJSON($jsonSolicitudCateo)) {
                ...
                if (!$errorGeneral) {
                    ...
                    return $SolicitudescateosDto = $SolicitudescateosController->insertSolicitudescateos($arraySolicitudCateo, null, "webService");
                } else {
...
$server = new SoapServer("ExhortosElectronicosServerScramble.wsdl");
$server->setClass("ExhortosElectronicosServer");
$server->handle();
