    ...
    public function selectSolicitudesaudiencias($SolicitudesaudienciasDto, $param) {
        ...
        if ($SolicitudesaudienciasDto != "") {
            ...
            foreach ($SolicitudesaudienciasDto as $Solicitudaudiencia) {
                ...
                $json .= '"fechaRegistro":' . json_encode(utf8_encode($this->fechaNormal($fecha[0]) . " " . $fecha[1])) . ",";
...
//            $json .= "}";
    ...
    public function actualizadmin($SolicitudesaudienciasDto) {
        $SolicitudesaudienciasDto = $this->validarSolicitudesaudiencias($SolicitudesaudienciasDto);
        $SolicitudesaudienciasController = new SolicitudesaudienciasController();
        $SolicitudesaudienciasDto = $SolicitudesaudienciasController->actualizadmin($SolicitudesaudienciasDto, "");
        return $SolicitudesaudienciasDto;
    }
...
if (($accion == "guardar")) {
...
} else if (($accion == "actualizadmin")) {
    $solicitudesaudienciasDto = $solicitudesaudienciasFacade->actualizadmin($solicitudesaudienciasDto);
    echo $solicitudesaudienciasDto;
    ...
    if ($manual === "S") {
        ...
        $solicitudesaudienciasDto->setCveUsuario($cveUsuario);
        if ($cveJuzgado === "") {
            $solicitudesaudienciasDto->setCveJuzgado($_SESSION['cveAdscripcion']);
        }
    }
