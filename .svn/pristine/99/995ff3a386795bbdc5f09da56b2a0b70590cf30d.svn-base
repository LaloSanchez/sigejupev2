        ...
        switch ($datos["nivel"]) {
            case 1:
                if ($datos["cveJuzgador"] == 0 && $datos["filtroPorJuez"] == true) {
                    ...
                    $datos["groups"] = "c.cveEtapaProcesal";
            ...
            case 5:
//                if ($datos["filtroPorJuez"] == true) {
//                    $datos["groups"] = "a.idAudiencia,jz.idJuzgador";
//                } else if ($datos["audienciasDeJuicio"] == true && $datos["porJuez"] == false) {
//                    $datos["groups"] = "a.idAudiencia,ep.cveEtapaProcesal";
//                } else if ($datos["porJuez"] == true && $datos["audienciasDeJuicio"] == true) {
//                    $datos["groups"] = "a.idAudiencia,jz.idJuzgador";
//                } else if ($datos["promedioDuracionAudiencias"] == true) {
//                    $datos["groups"] = "c.idAudiencia";
//                    if ($datos["filtroPorEtapaProcesal"] == true) {
//                        $datos["groups"].= ",c.cveEtapaProcesal";
//                    }
//                } else if ($datos["audienciasAJDC"] == true) {
//                    $datos["groups"] = "a.idAudiencia,ta.cveTipoAudiencia";
//                } else {
//                    $datos["groups"] = "a.idAudiencia";
//                }
                break;
        }
    ...
    public function reporteAudiencias($audienciasDto, $datos, $paginacion) {
        ...
        if ($datos["detalles"] == "detalle") {
            $params["fields"] = "cata.desCatAudiencia,tc.desTipoCarpeta,a.detenido,sala.desSala,";
        ...
        $params["tables"] = "tbltiposcarpetas tc,tblsalas sala,tblaudiencias a 
        ...
        $params["conditions"] = "sala.cveSala=a.cveSala AND sala.activo='S' AND tc.cveTipoCarpeta=a.cveTipoCarpeta AND tc.activo='S' AND a.activo='S' AND cata.activo='S' AND sa.activo='S' AND d.activo='S' AND r.activo='S' 
        ...
        if ($datos["filtroPorJuez"] == true) {
            ...
            if ($datos["cveJuzgador"] != 0) {
                $params["conditions"].=" AND jz.activo='S' AND aj.activo='S' AND jz.idJuzgador=" . $datos["cveJuzgador"];
            } else {
                $params["conditions"].=" AND jz.activo='S' AND aj.activo='S'";
            }
            if ($datos["cveTipoJuzgador"] != 0) {
                $params["conditions"].=" AND jz.cveTipoJuzgador=" . $datos["cveTipoJuzgador"];
            }
        ...
        if ($datos["audienciasAJDC"] == true) {
            ...
            $params["conditions"] .= " AND ep.activo='S' AND ea.activo='S' AND ta.activo='S' AND n.activo='S' AND aj.activo='S' AND jz.activo='S' 
...
//            $params["conditions"] .= " AND ep.activo='S' AND n.activo='S' AND (sa.cveEtapaProcesal=5 || sa.cveEtapaProcesal=2 || (sa.cveEtapaProcesal=1 AND sa.cveNaturaleza=2) || cata.audienciaInicial='S')";     
        ...
        if ($datos["audienciasDeJuicio"] == true) {
            ...
            if ($datos["porJuez"] == true) {
                ...
                $params["conditions"].=" AND jz.activo='S' AND aj.activo='S' AND cata.cveEtapaProcesal=" . $datos["cveEtapaProcesal"];
                if ($datos["cveJuzgador"] != 0) {
        ...
        if ($datos["promedioDuracionAudiencias"] == true) {
            ...
            if ($datos["cveEtapaProcesal"] != 0) {
                $etapa_procesal.=" AND cata.cveEtapaProcesal=" . $datos["cveEtapaProcesal"];
            }

            $regs = " a.idAudiencia,a.numero,a.anio,a.fechaInicial,a.fechaFinal,
            tj.cveTipoJuzgado,tj.desTipoJuzgado,j.desJuzgado,j.cveJuzgado,r.desRegion,r.cveRegion,
            d.cveDistrito,d.desDistrito,TIME_TO_SEC(TIMEDIFF(a.fechaFinal,a.fechaInicial)) AS diferencia,SEC_TO_TIME(TIME_TO_SEC(TIMEDIFF(a.fechaFinal,a.fechaInicial))) AS duracionAudiencia";
            $tablas = " tblaudiencias a LEFT JOIN tbljuzgados j ON a.cveJuzgado = j.cveJuzgado    
            LEFT JOIN tbltiposjuzgados tj ON j.cveTipojuzgado = tj.cveTipoJuzgado
            LEFT JOIN tbldistritos d ON j.cveDistrito  = d.cveDistrito
            LEFT JOIN tblregiones r ON j.cveRegion = r.cveRegion 
            INNER JOIN tblcataudiencias cata ON a.cveCatAudiencia=cata.cveCatAudiencia 
            INNER JOIN tblsolicitudesaudiencias sa ON a.idSolicitudAudiencia=sa.idSolicitudAudiencia
            INNER JOIN tbletapasprocesales ep ON cata.cveEtapaProcesal=ep.cveEtapaProcesal";
            $condiciones = " a.activo='S' AND d.activo='S' AND r.activo='S' AND sa.activo='S' AND ep.activo='S'
            AND tj.activo='S' AND j.activo='S' AND cata.activo='S' " . $cveJuzgado . $anio . $mes . $rango_fecha . $etapa_procesal . $region . $distrito . $tipo_juzgado;
            if ($datos["nivel"] == 5) {
                $params["fields"] = "c.idAudiencia,c.numero,c.anio,c.fechaInicial,c.fechaFinal,c.cveTipoJuzgado,c.desTipoJuzgado,c.desJuzgado,c.cveJuzgado,c.desRegion,c.cveRegion,
            c.cveDistrito,c.desDistrito,c.duracionAudiencia";
            } else {
                $params["fields"] = " COUNT(DISTINCT(c.idAudiencia)) as totalCount,SEC_TO_TIME(SUM(c.diferencia)/COUNT(c.idAudiencia)) AS promedioTotal,"
                        . "c.idAudiencia,c.numero,c.anio,c.fechaInicial,c.fechaFinal,c.cveTipoJuzgado,c.desTipoJuzgado,c.desJuzgado,c.cveJuzgado,c.desRegion,c.cveRegion,
            c.cveDistrito,c.desDistrito,c.duracionAudiencia";
            }
            if ($datos["filtroPorEtapaProcesal"] == true) {
                $params["fields"].=",c.cveEtapaProcesal,c.desEtapaProcesal";
                $regs.=",ep.cveEtapaProcesal,ep.desEtapaProcesal";
            }
