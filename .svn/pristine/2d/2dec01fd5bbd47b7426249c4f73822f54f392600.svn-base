...
//        print_r($ProgramacionsalasDto);
    ...
    /*
     * M�todo que permite obtener el detalle de la programaci�n mensual de salas
     * por juzgado, mes y a�o, rec�be como par�metro un objeto de tipo Programaciones
     * para obtener el id de la Programaci�n para el mes, Juzgado y a�o enviados
...
//        print_r($datos);
        ...
        $msg = array();
        /*
         * Revisar si ya existe la programaci�n de acuerdo a los datos enviados, si ya existe
        ...
        //calcular los d�as que hay entre fechaInicio y fechaFin
...
//        echo "despues de select VALUE ->" . $datos . "<- VALUE";
        if (!$datos) {
             ...
             * de los d�as que hay entre fechaInicio y fechaFin
             */
            for ($n = 0; $n < $dias; $n++) {
                ...
                /*
                 * si el d�a de la semana es lunes, se incrementa en 1 la variable
             ...
             * la disponibilidad de cada sala, en los d�as establecidos
             */
            $result = array();
            for ($a = 0; $a < count($dtoDias); $a++) {
                 ...
                 * haber registros, se env�an los datos obtenidos de la configuracion de salas
                 * si exisrte el registro, se env�an los datos de fecha y hora registrados
                 * en la base de datos
                 */
                ...
                //agregamos el d�a de la semana incluyendolo en la descripcon del juzgado
    ...
    public function programacionSalas($ProgramacionesDto, $proveedor = null){
        ...
        $logger->w_onError("**********COMIENZA PROCESO TRANSACCIONAL DE PROGRAMACI�N DE SALAS**********");
        if ($proveedor == null) {
            $this->proveedor = new Proveedor('mysql', 'sigejupe');
            $this->proveedor->connect();
    ...
    public function programacionSalasManual($programacionesDto, $programacionSalasDto, $proveedor = null){
        ...
        $dias = array();
        $logger = new Logger("/../../logs/", "ProgramacionSalas");
        $logger->w_onError("**********COMIENZA PROCESO TRANSACCIONAL DE PROGRAMACI�N MANUAL DE SALAS**********");
        if ($proveedor == null) {
            $this->proveedor = new Proveedor('mysql', 'sigejupe');
            $this->proveedor->connect();
            $this->proveedor->execute("BEGIN");
                ...
                if(count($idHorario) > 0){
                    ...
                    if(count($dias) > 0){
                        for($n = 0; $n < count($dias); $n++){
                            ...
                            if($programacionsalasDto == ""){
                                $logger->w_onError("**********SE DETERMINA QUE EL REGISTRO PARA EL D�A " . $dias[$n] . " NO EXISTE, POR LO TANTO SE INSERTARA");
                                //echo "<br>Se inserta: " . $dto[$n]->getIdProgramacion() . " - " . $dto[$n]->getCveJuzgado() . " - " . $dto[$n]->getCveSala() . " - " . $dto[$n]->getFechaInicio() . " - " . $dto[$n]->getFechaTermino();
                                ...
                                if($programacionesSalasDto == ""){
                                    $error = true;
                                    $logger->w_onError("**********OCURRIO UN ERROR AL ACTUALIZAR EL REGISTRO");
                                    ...
                                    $cveAccion = 18;
                                    $descripcion = "UPDATE tabla tblprogramacionsalas registro original: " . json_encode($programacionsalasDto[0]->toString()) . ", registro modificado: " . json_encode($programacionesSalasDto[0]->toString());
                ...
                for($n = 0; $n < count($programacionsalasDto); $n++){
                    ...
                    if(in_array($fechaTmp, $dias)){
                        ...
                        if($eliminar){
                            ...
                            $descripcion = "borrado fisico en tblprogramacionsalas: " . json_encode($programacionsalasDto[$n]->toString());
                            $bitacoramovimientosDto = new BitacoramovimientosDTO();
                            $bitacoramovimientosDto->setCveAccion($cveAccion);
                            $bitacoramovimientosDto->setFechaMovimiento($fecha);
                            $bitacoramovimientosDto->setObservaciones($descripcion);
