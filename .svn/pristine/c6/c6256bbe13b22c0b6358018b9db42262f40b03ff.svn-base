...
if (isset($_SESSION["cveUsuarioSistema"]) && $_SESSION["cveUsuarioSistema"] !== "") {
    ...
    <div class="panel panel-default">
        <div class="panel-heading">
            <h5 class="panel-title" id="h5titulo">                                                            
                Reporte de Cateos en el Estado de M&eacute;xico.
            </h5>
        </div>
            ...
            <div id="divFormulario" class="form-horizontal">
                <div id="divPanelControl">
                    <div class="form-group">                                                                
                        <label class="control-label col-md-4" id="lblRelationName">A&ntilde;o del Cateo</label>
                    ...
                    <div class="form-group">                                                                
                        <label class="control-label col-md-4" >Mes (Solicitud)</label>
                    ...
                    <div class="form-group">                                                                
                        <label class="control-label col-md-4" >Rango de fecha (Solicitud)</label>
                    ...
                    <div class="form-group" id="divEstatussolicitudcateo"> 
                        <label class="control-label col-md-4">Estatus de Cateo</label>
                        <div class="col-md-5">
                            <div id="divCmbEstatussolicitudcateo" class="logobox"></div>
                            <select class="form-control Relacionado" name="cmbEstatussolicitudcateo" id="cmbEstatussolicitudcateo">
                                <option value="0">Seleccione una opci&oacute;n</option>
                            </select>
                        </div>                                
                    </div>
                    <div class="form-group" id="divRespuestacateo"> 
                        <label class="control-label col-md-4">Respuesta de Cateo</label>
                        <div class="col-md-5">
                            <div id="divCmbRespuestacateo" class="logobox"></div>
                            <select class="form-control Relacionado" name="cmbRespuestacateo" id="cmbRespuestacateo">
                                <option value="0">Seleccione una opci&oacute;n</option>
                            </select>
                        </div>                                
                    </div>
    ...
    <script type="text/javascript">
        ...
        exportar = function (accion, div) {
            ...
            var nombreArchivo = "REPORTE_CATEOS";
        ...
        limpiar = function () {
            ...
            $("#divGrafica").hide();
            $("#paginacion").hide();
            $("#h5titulo").html("<span style='text-decoration: underline; cursor:pointer;' onclick='regresar(0,false);'>Reporte de Cateos en el Estado de M&eacute;xico.</span>");
        };
        ...
        filtro = function () {
            ...
            //        }
            if ($("#cmbEstatussolicitudcateo").val() != 0) {
                strDatos += "&cveEstatusSolicitudCateo=" + $("#cmbEstatussolicitudcateo").val();
            }
            if ($("#cmbRespuestacateo").val() != 0) {
                strDatos += "&cveRespuestaSolicitudCateo=" + $("#cmbRespuestacateo").val();
            }
        ...
        titulos = function () {
            ...
            var titulo = "Reporte de Cateos" + etiqueta;
            if ($("#txtAnio").val() != "") {
        ...
        gestorNivel = function (json, bandera) {
            ...
            switch ($("#hiddenNivel").val()) {
                ...
                case "5":
                    ...
                    table += "<th>Cateo</th><th>Juez</th><th>Fecha de Solicitud</th><th>Estatus</th><th>Respuesta</th>";
                    table += "</tr></thead><tbody>";
                    for (var i = 0; i < json.totalCount; i++) {
                        table += "<tr>";
                        table += "<td> " + (i + 1 + indice) + "</td>";
                        ...
                        table += "<td>CATEO: " + json.resultados[i].numeroCateo + "/" + json.resultados[i].anioCateo + "</td>";
                        ...
                        table += "<td>" + json.resultados[i].desEstatusSolicitudCateo + "</td>";
                        table += "<td>" + json.resultados[i].desRespuestaSolicitudCateo + "</td>";
                        table += "</tr> ";
                        subTotal = -(i + 1 + indice);
                        aux = "";
                    }
        ...
        consultar = function (bandera, nivel) {
            ...
            var strDatos = "accion=consultar_Cateos_Reporte";
            strDatos += "&activo=S";
            ...
            $.ajax({
                type: "POST",
                url: "../fachadas/sigejupe/reportes/ReporteCateosFacade.Class.php",
                data: strDatos,
                async: true,
                dataType: "html",
    ...
    //        }
        ...
        calcularPaginas = function () {
            $("#divRegresar").hide();
            var url = "../fachadas/sigejupe/reportes/ReporteCateosFacade.Class.php";
            var strDatos = "accion=consultar_Cateos_Reporte";
        ...
        encabezado = function () {
            ...
            $("#h5titulo").html("<span style='text-decoration: underline; cursor:pointer;' onclick='regresar(0,false);'>Reporte de Cateos en el Estado de M&eacute;xico.</span>" + mensaje);
        };
        regresar = function (nivel, bandera) {
            ...
            var strDatos = "";
            if (nivel == 0) {
                $("#h5titulo").html("Reporte de Cateos en el Estado de M&eacute;xico.");
                $("#divConsulta").hide("slide");
                $("#paginacion").hide();
                $("#inputImprimir").hide();
                $("#inputExportarPDF").hide();
        ...
        //        });
        //    };
        cargarEstatusCateo = function () {
            var strDatos = "accion=consultarOrdenado";
            strDatos += "&activo=S";
            strDatos += "&orden= order by desEstatusSolicitudCateo";
            $.ajax({
                type: "POST",
                url: "../fachadas/sigejupe/estatussolicitudescateos/EstatussolicitudescateosFacade.Class.php",
                data: strDatos,
                async: true,
                dataType: "html",
                beforeSend: function (objeto) {
                ...
                success: function (datos) {
                    ...
                    if (json.totalCount > 0) {
                        for (var i = 0; i < json.totalCount; i++) {
                            if (json.data[i].cveEstatusSolicitudCateo != 4) {
                                $("#cmbEstatussolicitudcateo").append($('<option></option>').val(json.data[i].cveEstatusSolicitudCateo).html(json.data[i].desEstatusSolicitudCateo));
                            }
                        }
        ...
        cargarRespuestaCateo = function () {
            var strDatos = "accion=consultar";
            strDatos += "&activo=S";
            // strDatos += "&orden= order by desEstatusSolicitudCateo";
            $.ajax({
                type: "POST",
                url: "../fachadas/sigejupe/respuestasolicitudcateo/RespuestasolicitudcateoFacade.Class.php",
                data: strDatos,
                async: true,
                dataType: "html",
                beforeSend: function (objeto) {
                ...
                success: function (datos) {
                    ...
                    if (json.totalCount > 0) {
                        for (var i = 0; i < json.totalCount; i++) {
                            if (json.data[i].cveEstatusSolicitudCateo != 4) {
                                $("#cmbRespuestacateo").append($('<option></option>').val(json.data[i].cveRespuestaSolicitudCateo).html(json.data[i].desRespuestaSolicitudCateo));
                            }
                        }
                                ...
                                $.each(response.perfiles[0].perfil[i].permisos, function (k, vnombre) {
                                    if (vnombre.nomFormulario == "REPORTES") {
                                        ...
                                        $.each(hijos, function (k2, nombreHijo) {
                                            if (nombreHijo.nomFormulario == 'CATEOS') {
