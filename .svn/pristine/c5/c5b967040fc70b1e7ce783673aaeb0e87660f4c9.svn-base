        ...
        if ((!$error) && (count($msg) == 0)) {
            ...
            if ((int) $SolicitudesaudienciasDto->getIdSolicitudAudiencia() <= 0) {
                ...
                if (((int) $SolicitudesaudienciasDto->getCveTipoCarpeta() <= 0) || ((string) $SolicitudesaudienciasDto->getCveTipoCarpeta() == "")) {
                    ...
                    $orden = " " . (($SolicitudesaudienciasDto->getCarpetaInv() != "") ? " And carpetaInv='" . $SolicitudesaudienciasDto->getCarpetaInv() . "' " : " ") . " " . (($SolicitudesaudienciasDto->getNuc() != "") ? " " . (($SolicitudesaudienciasDto->getCarpetaInv() != "") ? "OR" : "AND") . " nuc='" . $SolicitudesaudienciasDto->getNuc() . "' " : " ");
                    $logger->w_onError($orden);
...
///////////Modificacion kary
                    ...
                    if ($carpetasJudicialesDto != "") {
                        foreach ($carpetasJudicialesDto as $carpetasJudiciales) {
                            if ($carpetasJudiciales->getCveJuzgado() == $SolicitudesaudienciasDto->getCveJuzgado()) {
                                $existe = true;
                                $carpetasJudicialesDto = $carpetasJudiciales;
                                break;
                            }
                        }
                    } else {
                    ...
                    if (!$existe) {
                        $carpetasJudicialesDto = array();
                        $msg[] = array("La carpeta de investigacion o NUC pertenecen a un juzgado distinto al que se esta solicitando.");
                        $error = true;
                    }
///////////FIN Modificacion kary
...
//                                            ImputadosdrogasDTO ImputadosdrogascarpetasDTO
                                                    ...
                                                    if ($ofendidoscarpetasDto != "") {
                                                        for ($index = 0; $index < count($ofendidoscarpetasDto); $index ++) {
                                                            ...
                                                            if ((int) ($ofendidoscarpetasDto[$index]->getCveTipoPersona()) === 1 || ($ofendidoscarpetasDto[$index]->getCveTipoPersona()) === 4 || ($ofendidoscarpetasDto[$index]->getCveTipoPersona()) === 5) {
...
//                                                }
                                                ...
                                                $msg[] = array("Error. No existe la relacion en la carpeta (Paso 5), por favor verifique.");
                                                $error = true;
                                            }
                                        }
                                    }
