...
$this->proveedor = new Proveedor('mysql', 'sigejupe');
        ...
        if ($reporte == "CausaJuicio") {
            $campos .= " ,js.nombre,js.titulo,js.paterno,js.materno ,year(cj.fechaRadicacion) as year ,month(cj.fechaRadicacion) as mes";
        ...
        if ($datos["porMes"] != "") {
            ...
            if ($datos["anio1"] != "") {
                ...
                if ($condiciones != "") {
                    ...
                    $condiciones.=" AND year(cj.fechaRadicacion)=" . $datos['anio1'];
                } else {
                    $condiciones.="  year(cj.fechaRadicacioasdana)=" . $datos['anio1'];
        }
            }
        }


        $params["fields"] .= "ec.desEstatusCarpeta,cj.cveEstatusCarpeta, cj.idCarpetaJudicial,tj.cveTipoJuzgado,tj.desTipoJuzgado,cj.numero,cj.anio,r.desRegion,r.cveRegion,d.desDistrito,d.cveDistrito,cj.fechaRadicacion,"
        ...
        $params["conditions"] = "cj.activo = 'S' AND j.activo = 'S' AND tj.activo = 'S' AND ec.activo = 'S' AND r.activo = 'S' AND d.activo = 'S' AND tc.activo = 'S' AND tj.activo = 'S' AND cj.cveTipoCarpeta <= 5  " . $condiciones;
...
//                if ($datos["nivel"] == "1") {
        ...
        foreach($rs->resultados as $key => $value){
         ...
         foreach($value as $key2 => $value2){
            ...
            if($datos["nivel"] == 5){
            if ($paginacion["paginacion"] != "false") {
            $ArrCausas[$key]["codigo"] = $this->obtenerCodigo($numero,$anio,$cveJuzgado,$paginacion["paginacion"],$this->proveedor);//
            } 
            }
        }
    ...
    public function obtenerCodigo($numero,$anio,$cveJuzgado,$nivel,$proveedor){
            ...
            $AudienciasDto = $AudienciasController->consultarAudienciaCodigo($AudienciasDto,$proveedor);
