...
//                            "materno" => utf8_encode($quejoso->getMaternoQ()),
                ...
                $resultado = array(
                    ...
                    "cveEstatusAmparo" => $amparo->getCveEstatusAmparo(),
    ...
    public function insertAmparos($AmparosDto, $proveedor = null, $listaQuejosos, $listaTerceros, $tipoPersonaAdd) {
        ...
        $contadoresDto->setAnio($AmparosDto->getAniAmparo()); // A�O EN EL QUE SE VA A BUSCAR EL CONTADOR
        $contador = $contadorDao->selectContadores($contadoresDto, "", $proveedor);

        #SI EL A�O DEL AMPARO ES MAYOR O  IGUAL AL A�O ACTUAL SE VALIDA EL CONTADOR, EN OTRO CASO SE GUARDA COMO MANDEN EL NUMERO Y A�O
        if ($AmparosDto->getAniAmparo() >= date("Y")) {
            if ($contador != "") {
                ...
                $transaccion = 0;
                error_log("No esta configurado el contador");
                $mensaje .= "NO SE ENCONTRO LA CONFIGURACI�N DE LOS CONTADORES PARA EL ANIO ACTUAL";
            }
        }
...
//                        $terceroDto->setNombreMoral($tercero["nombre"]);
            ...
            $transaccion = 0;
            error_log("ya existia amparo con numero y a�o proporcionado");
