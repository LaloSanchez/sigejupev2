        ...
        if(!$error) {
            ...
            //if($programacionMuestrasTmp == "") {
                ...
                if($programacionMuestrasDto != "") {
                    ...
                    $bitacoramovimientosDto = new BitacoramovimientosDTO();
                    $cveAccion = 367;
                    $descripcion = "UPDATE tabla tblprogramacionmuestras idProgramacion: " . $programacionMuestrasDto[0]->getIdProgramacionMuestra();
                    $bitacoramovimientosDto->setCveAccion($cveAccion);
    ...
    public function juezTomaDeMuestra($juzgadoDto, $programacionMuestrasDto, $proveedor = null){
        ...
        if ( $gruposMuestrasJuzgadosDto != "" ) {
            foreach ( $gruposMuestrasJuzgadosDto as $grupoJuzgado ) {
                 ...
                 * juez está disponible para realizar la toma de muestra
                 */
                $programacionMuestrasDto = new ProgramacionmuestrasDTO();
                $programacionMuestrasDao = new ProgramacionmuestrasDAO();
                //$programacionMuestrasDto->setCveGrupoMuestraJuzgado($grupoJuzgado->getCveGrupoMuestraJuzgado());
...
//                        echo "</pre>";
                    ...
                    $error = true;
                    $msg = " No se encontraron jueces programados para atender toma de muestras para el juzgado " . utf8_encode($juzgadoTmp[0]->getDesJuzgado()) . " en el día " . $fechaInicio[0] . " a las " . $fechaInicio[1];
                }
            }
        } else {
            $error = true;
            $msg = " No se encontraron datos de programación de toma de muestras para el juzgado " . utf8_encode($juzgadoTmp[0]->getDesJuzgado());
        }
        if ( !$error ) {
            $datos = array(
                "estatus"    => "ok",
