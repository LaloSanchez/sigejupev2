...
if (isset($_SESSION["cveUsuarioSistema"]) && $_SESSION["cveUsuarioSistema"] !== "") {
    ...
    include_once(dirname(__FILE__) . "/../../../controladores/sigejupe/cateos/CateosController.Class.php");
    include_once(dirname(__FILE__) . "/../../../controladores/sigejupe/cateos/ComprobanteCateosController.Class.php");
    include_once(dirname(__FILE__) . "/../../../controladores/sigejupe/apelacioncateos/ComprobanteApelacioncateosController.Class.php");
    ...
    class CateosFacade {
        ...
        public function selectCateos($CateosDto) {
            ...
            $CateosController = new CateosController();
            $CateosDto = $CateosController->selectCateos($CateosDto);
            if ($CateosDto != "") {
        ...
        public function insertCateos($param = "") {
            $CateosController = new CateosController();
            return $CateosDto = $CateosController->insertRespuestaCateo($param);
            /* $CateosDto = $this->validarCateos($CateosDto);
              $CateosController = new CateosController();
              $CateosDto = $CateosController->insertCateos($CateosDto);
              if ($CateosDto != "") {
              $dtoToJson = new DtoToJson($CateosDto);
        ...
        public function updateCateos($CateosDto) {
            $CateosDto = $this->validarCateos($CateosDto);
            $CateosController = new CateosController();
            $CateosDto = $CateosController->updateCateos($CateosDto);
            if ($CateosDto != "") {
                $dtoToJson = new DtoToJson($CateosDto);
        ...
        public function deleteCateos($CateosDto) {
            $CateosDto = $this->validarCateos($CateosDto);
            $CateosController = new CateosController();
            $CateosDto = $CateosController->deleteCateos($CateosDto);
            if ($CateosDto == true) {
                $jsonDto = new Encode_JSON();
...
//                    readfile("./../../../solicitudespdf/" . $resultado["file"]);
        ...
        public function descargaResolucionSecretario($idCateo) {
            if ($idCateo != "" && $idCateo != "0") {
                $ComprobanteApelacioncateosController = new ComprobanteApelacioncateosController();
                $resultado = $ComprobanteApelacioncateosController->generaComprobante($idCateo);
                if ($resultado != "") {
                    if (!$resultado["type"] == "OK") {
//                    #DESCARGA LA SOLICITUD GENERADA
//                        header("Content-disposition: attachment; filename=" . $resultado["fileName"]);
...
//                        readfile("./../../../solicitudespdf/" . $resultado["filePath"]);
        ...
        public function descargaResolucionSecretarioPDF($idCateo) {
            if ($idCateo != "" && $idCateo != "0") {
                $ComprobanteApelacioncateosController = new ComprobanteApelacioncateosController();
                $resultado = $ComprobanteApelacioncateosController->generaComprobante($idCateo);
                if ($resultado != "") {
...
//                            "file" => "");
        ...
        public function getCateosPendientes() {
            $ctrCateos = new CateosController();
            $result = "";
    ...
    if (isset($_SESSION["cveUsuarioSistema"])) {
        if (($accion == "guardar") && ($idCateo == "")) {
            $cateosDto = $cateosFacade->insertCateos($cateosDto);
            echo $cateosDto;
        } else if (($accion == "guardar") && ($idCateo != "")) {
            $cateosDto = $cateosFacade->updateCateos($cateosDto);
            echo $cateosDto;
        } else if ($accion == "consultar") {
            $cateosDto = $cateosFacade->selectCateos($cateosDto);
            echo $cateosDto;
            exit;
        } else if (($accion == "baja") && ($idCateo != "")) {
            $cateosDto = $cateosFacade->deleteCateos($cateosDto);
            echo $cateosDto;
            ...
            $cateosDto = $cateosFacade->insertCateos($param);
            echo json_encode($cateosDto);
        } else if (($accion == "descargaSolicitudCateo")) {
            $solicitudescateosDto = $cateosFacade->descargaSolicitudCateo($idCateo);
            echo $solicitudescateosDto;
        } else if (($accion == "descargaResolucionSecretario")) {
            $solicitudescateosDto = $cateosFacade->descargaResolucionSecretario($idCateo);
            echo $solicitudescateosDto;
        } else if (($action == "descargaResolucionSecretario")) {
            @$idCateo = $_GET["idCateo"];
            $solicitudescateosDto = $cateosFacade->descargaResolucionSecretarioPDF($idCateo);
            exit;
        } else if ($action == "descargaRespuestaCateo") {
            echo $idCateo = $_GET['idCateo'];
            $cateosFacade->descargaRespuestaCateo($idCateo);
            exit;
        } else if ($accion == "getCateosPendientes") {
            echo $cateosFacade->getCateosPendientes();
        } else if ($accion == "getInfoJuzgadorWS") {
            $idJuzgado = $_POST["juzgado"];
            echo $cateosFacade->getInfoJuzgadorWS($idJuzgado);
        }
    } else {
