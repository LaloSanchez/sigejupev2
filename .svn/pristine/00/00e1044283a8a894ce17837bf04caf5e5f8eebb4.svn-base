...
//                }
             ...
             */
            if (!$error) {
                $registroAnterior  = $dto[0]->toString();
                $registroNuevo = $carpetasJudicialesDto->toString();
                $registroAnterior['observaciones'] = utf8_encode($registroAnterior['observaciones']);
                $registroNuevo['observaciones'] = utf8_encode($registroNuevo['observaciones']);
//                var_dump(json_encode($registroAnterior));
//                var_dump(json_encode($registroNuevo));
                $descripcion = "UPDATE tblcarpetasjudiciales, Registro antes de Modificar: " . json_encode($registroAnterior) . ", Registro Modificado: " . json_encode($registroNuevo);
