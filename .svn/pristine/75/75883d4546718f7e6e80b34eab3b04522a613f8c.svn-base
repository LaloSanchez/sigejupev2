...
//////////////////////////TESTIGOS
    ...
    public function guardaEfectosViolencia($ViolenciadegeneroDto, $EfectosofendidosDto, $proveedor = null, $usaBitacora = true) {
        ...
        if ($rsImpofedelSolicitudes != "") {
            ...
            $solicitudAudienciasDto->setIdSolicitudAudiencia($rsImpofedelSolicitudes[0]->getIdSolicitudAudiencia());
            ...
            if ($rsSolicitudAudiencia != "") {
                if ($rsSolicitudAudiencia[0]->getCveEstatusSolicitud() == 1) {
                    ...
                    $result = array("status" => "error", "mnj" => "La solicitud de audiencia se encuentra enviada o cancelada. Verifique");
                    $result = json_encode($result);
                }
            } else {
                $result = array("status" => "error", "mnj" => "La solicitud de audiencia no existe, Verifique");
                $result = json_encode($result);
            }
        } else {
        ...
        if ($rsImpofedelSolicitudes != "") {
            ...
            if ($rsSolicitudAudiencia != "") {
                if ($rsSolicitudAudiencia[0]->getCveEstatusSolicitud() == 1) {
                    ...
                    $result = array("status" => "error", "mnj" => "La solicitud de audiencia no se puede modificar ya que se encuentra enviada o cancelada. Verifique");
                    $result = json_encode($result);
                }
            } else {
                $result = array("status" => "error", "mnj" => "La solicitud de audiencia no existe, Verifique");
...
//                PRINT_R($rsEfectosofendidosDto);
    ...
    public function eliminarEfecto($ViolenciadegeneroDto, $EfectosofendidosDto, $proveedor = null, $usaBitacora = true) {
        ...
        if ($rsViolenciaDeGenero != "") {
            ...
            //////////Verificamos que exista la relacion
            $impofedelSolicutdesDto->setIdImpOfeDelSolicitud($rsViolenciaDeGenero[0]->getIdImpOfeDelSolicitud());
            $rsImpofedelSolicitudes = $impofedelSolicutdesDao->selectImpofedelsolicitudes($impofedelSolicutdesDto);
            if ($rsImpofedelSolicitudes != "") {
                $solicitudAudienciasDto = new SolicitudesaudienciasDTO();
                $solicitudAudienciasDao = new SolicitudesaudienciasDAO();
...
//            print_r($totalActivosEfectos);
            ...
            $result = array("status" => "error", "mnj" => "La violencia de genero no existe, Verifique");
            $result = json_encode($result);
        }
        return $result;
    }
