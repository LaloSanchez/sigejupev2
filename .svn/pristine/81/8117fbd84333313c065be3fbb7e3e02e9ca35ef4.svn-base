...
if ($solicitud->getFachada() == "index") {
    ...
    if (file_exists($accion)) {
		...
		$catPermitidos = array("tiposdetenciones","delitos","cataudiencias","estadosciviles","nivelesinstrucciones","ocupaciones","municipios","estados","drogas","gruposednicos","catdelitos","alfabetismo","bienesjuridicosafectados","clasificacionesdelitos","clasificacionesdelitosorden","codigos","comisiones","concursos","condiciones","conductas","consignaciones","convivencias","dialectoindigena","distritos","elementoscomisiones","espanol","estadospsicofisicos","formasacciones","generos","gradoparticipaciones","interpretes","juzgados","materias","modalidades","paises","regiones","tiposreligiones");
		if(in_array($accion,$catPermitidos)){
                ...
                if ($type == "json") {
                    if ($dto != "") {
                        ...
                        echo utf8_encode($dtoToJson->toJson(""));
                    } else {
      ...
      <$accion>
                    ...
                    $xmlFormat = trim(utf8_encode($fileXml->asXML()));
                    echo $xmlFormat;
                    //                    echo "prueba";
                } else if ($type == "doc") {
                    $html = "<table border=\"1\">";
