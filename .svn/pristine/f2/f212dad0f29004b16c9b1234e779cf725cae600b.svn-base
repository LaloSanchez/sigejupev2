...
ini_set("error_log", dirname(__FILE__) . "/../../../logs/AmparosController.log");
...
//        $contadoresDto->set("8"); // Tipo de carpeta Amparo 8
        $contadoresDto->setAnio(date("Y"));

        $contador = $contadorCrl->getContador($contadoresDto, $proveedor);
        if ($contador != "") {
            $AmparosDto->setNumAmparo($contador[0]->getNumero());
            $AmparosDto->setAniAmparo($contador[0]->getAnio());
        } else {
            $transaccion = 0;
        ...
        /*
          aqui va a guardar el estatus de el amparo
         */
        if ($AmparosDto != "") {
