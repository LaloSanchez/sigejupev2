...
//            CURLOPT_URL => "http://sigedepu.idpedomexx.gob.mx:9000",
            ...
            if ($ofendidosSolicitudesDto != "") {
                ...
                if ($solicitudesAudienciasDto != "") {
                    ...
                    if ($DefensoresofendidossolicitudesDto != "") {
                        ...
                        $audienciasDto->setIdSolicitudAudiencia($ofendidosSolicitudesDto[0]->getIdSolicitudAudiencia());
                        $audienciasDto->setActivo("S");
                        $audienciasDto = $audienciasDao->selectAudiencias($audienciasDto);
                        if ($audienciasDto != "") {
                            list($fechaSolicitud, $horaSolicitud) = explode(" ", $audienciasDto[0]->getFechaInicial());
                            $param .= '"fechaAudiencia":' . json_encode(utf8_encode($this->fechaNormal($fechaSolicitud))) . ",";
                            $param .= '"horaAudiencia":' . json_encode(utf8_encode($horaSolicitud)) . ",";

                            //juzgados
                            $juzgadosDto->setCveJuzgado($audienciasDto[0]->getCveJuzgadoDesahoga());
                            $juzgadosDto = $juzgadosDao->selectJuzgados($juzgadosDto);
                            if ($juzgadosDto != "") {
                                $param .= '"juzgado":' . json_encode(utf8_encode($audienciasDto[0]->getCveJuzgadoDesahoga() . " - " . $juzgadosDto[0]->getDesJuzgado())) . ",";
                            } else {
                                $param .= '"juzgado": "",';
                            }
                        } else {
...
//$json = '{"idReferencia": "1267091","nombreDefensor": "Rodrigo","paternoDefensor": "García","maternoDefensor": "Palafox","emailDefensor": "r.palafox@anyplace.com","celularDefensor": "554647373","folioSolicitud": "IDP/EDOMEX/DRT/177/2016","estatus": "AUTORIZADO" }';
//$controlador = new DefensoresofendidossolicitudesController();
