...
Class ReporteCargaJuecesInicialController {
...
//sum(CASE WHEN (a.cveEstatusAudiencia =3) THEN 1 ELSE 0 END) cancelada, count(*) as total";
        ...
        if ($datos['detalles'] != "") {
            ...
            $condiciones .= " AND ca.cveCatAudiencia= a.cveCatAudiencia AND ca.activo='S'";
        }
        if($datos["cveCatAudiencia"] != ""){
            if ($condiciones != "") {
                $condiciones .= " AND a.cveCatAudiencia = ". $datos["cveCatAudiencia"];
            } else {
                $condiciones .= " AND a.cveCatAudiencia = ". $datos["cveCatAudiencia"];
            }
            
        }else{
            $condiciones .= " AND (a.cveCatAudiencia = 89 or a.cveCatAudiencia = 62 or a.cveCatAudiencia = 135) ";
        }
        if ($datos["cveJuzgado"] != "") {
            if ($condiciones != "") {
        ...
        $params["conditions"] =  /*js.activo='S' AND*/" a.cveJuzgado=j.cveJuzgado AND s.activo='S' AND a.idSolicitudAudiencia= sa.idSolicitudAudiencia AND a.activo='S' 
AND sa.activo='S'
