...
class ValidaSolicitudOfendido {
    ...
    public function validarEstatusSolicitudByIdOfendido($idOfendidoSolicitud, $accion, $modulo, $proveedor = null)
    {
        ...
        try {
            ...
            $getOfendido = $ofendidosSolicitudesDao->selectOfendidossolicitudes($ofendidosSolicitudesDto);

            if ($getOfendido == '') throw new Exception('No se encontro el ofendido para obtener su solicitud y validar el estatus de la misma');
            ...
            if ($getSolicitud == '') throw new Exception('no se encontr&oacute; la solicitud para validar su estatus.');
