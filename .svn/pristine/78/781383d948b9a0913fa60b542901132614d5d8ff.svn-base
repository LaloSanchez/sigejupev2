...
include_once(dirname(__FILE__) . "/../../../modelos/sigejupe/dao/testigosactuaciones/TestigosactuacionesDAO.Class.php");
...
//        public function selectActuaciones($actuacionesDto, $proveedor = null, $orden = "", $param = null, $fields = null) {
        ...
        $ActuacionesDto = $ActuacionesDao->selectActuaciones($ActuacionesDto, null, " AND (cveTipoActuacion=2 OR cveTipoActuacion=5 OR cveTipoActuacion=6 OR cveTipoActuacion=34) ORDER BY idActuacion DESC ", null, null);
        //print_r($ActuacionesDto);
        if ($ActuacionesDto != "") {
            $json = "";
            $json .= '{"type":"OK",';
            ...
            foreach ($ActuacionesDto as $Actuacion) {
                ...
                if ($Actuacion->getFechaRegistro() != "") {
                    $fechaHora = explode(" ", $Actuacion->getFechaRegistro());
                    $fecha = explode("-", $fechaHora[0]);
                    $fechaRegistro = $fecha[2] . "/" . $fecha[1] . "/" . $fecha[0] . " " . $fechaHora[1];
                } else {
                    $fechaRegistro = "";
                }

                $json .= '"fechaRegistro":' . json_encode(utf8_encode($fechaRegistro)) . ",";
...
//                var_dump($ImputadoscarpetasDto);
    ...
    public function GuardarNotificacion($ActuacionesDto, $param) {
        ...
        if ($ActuacionesDto->getFechaDevolucion() != "") {
            $fecha = explode("/", $ActuacionesDto->getFechaDevolucion());
            $ActuacionesDto->setFechaDevolucion($fecha[2] . "-" . $fecha[1] . "-" . $fecha[0]);
        }
...
//                                        $transaccion = 0;
                        ...
                        if ($param["cadenaTestigos"] != "") {
                            $arrayTestigos = json_decode($param["cadenaTestigos"], true);
                            $testigosactuacionesDao = New TestigosactuacionesDAO();
                            foreach ($arrayTestigos as $testigo) {
                                $testigosactuacionesDto = New TestigosactuacionesDTO();
                                $testigosactuacionesDto->setIdActuacion($idActuacion);
                                $testigosactuacionesDto->setCveReferenciaTipoParte($testigo["cveReferenciaTipoParte"]);
                                $testigosactuacionesDto->setIdReferenciaParte($testigo["idReferenciaParte"]);
                                $testigosactuacionesDto->setCveTipoParte(16);
                                $testigosactuacionesDto->setNombreTestigo(utf8_decode($testigo["nombreTestigo"]));
                                $testigosactuacionesDto->setActivo("S");
                                $testigosactuacionesDto = $testigosactuacionesDao->insertTestigosactuaciones($testigosactuacionesDto, $proveedor);
                                if ($testigosactuacionesDto != "") {
//                                    $transaccion = 0;
                                } else {
                                    $transaccion = 1;
                                    echo "Tuve errores al insertar la notificación del DEFENSOR IMPUTADO";
...
//        public function selectActuaciones($actuacionesDto, $proveedor = null, $orden = "", $param = null, $fields = null) {
        ...
        if ($ActuacionesDto != "") {
            ...
            foreach ($ActuacionesDto as $actuacionDto) {
                ...
                if ($actuacionDto->getFechaDevolucion() != "") {
                    $fecha = explode("-", $actuacionDto->getFechaDevolucion());
                    $actuacionDto->setFechaDevolucion($fecha[2] . "/" . $fecha[1] . "/" . $fecha[0]);
                }

                $json .= '"fechaDevolucion":' . json_encode($actuacionDto->getFechaDevolucion()) . ",";
                $json .= '"diligencia":' . json_encode($actuacionDto->getDiligencia()) . ",";
...
//                $json .= '"observacionesPadre":' . json_encode(htmlentities($ActuacionesDtoPadre[0]->getObservaciones())) . ',';
                    ...
                    if ($ActuacionesDtoPadre[0]->getCveTipoActuacion() == 5) {
                        $Origen = "AUTO";
                    }
                    if ($ActuacionesDtoPadre[0]->getCveTipoActuacion() == 6) {
                        $Origen = "ACTA MINIMA";
                    }
                ...
                $testigosActuacionesDao = new TestigosactuacionesDAO();
                $testigosActuacionesDto = new TestigosactuacionesDTO();
                $testigosActuacionesDto->setIdActuacion($actuacionDto->getIdActuacion());
                $testigosActuacionesDto->setCveTipoParte(16);
                $testigosActuacionesDto->setActivo("S");

                $testigosActuacionesDto = $testigosActuacionesDao->selectTestigosactuaciones($testigosActuacionesDto);
                $json .= '"detalleTestigos": [';
                if ($testigosActuacionesDto != "" and $testigosActuacionesDto != null) {
                    foreach ($testigosActuacionesDto as $testigoActuacion) {
                        $json .= "{";
                        $json .= '"idTestigoActuacion":' . json_encode($testigoActuacion->getIdTestigoActuacion()) . ",";
                        $json .= '"idActuacion":' . json_encode($testigoActuacion->getIdActuacion()) . ",";
                        $json .= '"cveReferenciaTipoParte":' . json_encode($testigoActuacion->getCveReferenciaTipoParte()) . ",";
                        $json .= '"idReferenciaParte":' . json_encode($testigoActuacion->getIdReferenciaParte()) . ",";
                        $json .= '"cveTIpoParte":' . json_encode($testigoActuacion->getCveTipoParte()) . ",";
                        $json .= '"nombreTestigo":' . json_encode(utf8_encode($testigoActuacion->getNombreTestigo())) . ",";

                        if ($testigoActuacion->getCveReferenciaTipoParte() == 1) {
                            $ImputadoscarpetasDto = new ImputadoscarpetasDTO();
                            $ImputadoscarpetasDao = new ImputadoscarpetasDAO();
                            $ImputadoscarpetasDto->setIdImputadoCarpeta($testigoActuacion->getIdReferenciaParte());
                            $ImputadoscarpetasDto = $ImputadoscarpetasDao->selectImputadoscarpetas($ImputadoscarpetasDto);
                            $json .='"tipo": "IMPUTADO",';
                            if ($ImputadoscarpetasDto[0]->getCveTipoPersona() == 1) {
                                if ($ImputadoscarpetasDto[0]->getNombre() != "") {
                        ...
                        if ($testigoActuacion->getCveReferenciaTipoParte() == 2) {
                            $OfendidosCarpetasDto = new OfendidoscarpetasDTO();
                            $OfendidosCarpetasDao = new OfendidoscarpetasDAO();
                            $OfendidosCarpetasDto->setIdOfendidoCarpeta($testigoActuacion->getIdReferenciaParte());
                            $OfendidosCarpetasDto = $OfendidosCarpetasDao->selectOfendidoscarpetas($OfendidosCarpetasDto);
                        ...
                        if ($testigoActuacion->getCveReferenciaTipoParte() == 8) {
                            $ImputadosexhortosDto = new ImputadosexhortosDTO();
                            $ImputadosexhortosDao = new ImputadosexhortosDAO();
                            $ImputadosexhortosDto->setIdImputadoExhorto($testigoActuacion->getIdReferenciaParte());
                            $ImputadosexhortosDto = $ImputadosexhortosDao->selectImputadosexhortos($ImputadosexhortosDto);
                            $json .= '"idImputadoCarpeta":' . json_encode(utf8_encode($ImputadosexhortosDto[0]->getIdImputadoExhorto())) . ",";
                            $json .='"tipo": "IMPUTADO",';
                            if ($ImputadosexhortosDto[0]->getCveTipoPersona() == 1) {
                        ...
                        if ($testigoActuacion->getCveReferenciaTipoParte() == 9) {
                            $OfenfendidosexhortosDto = new OfenfendidosexhortosDTO();
                            $OfenfendidosexhortosDao = new OfenfendidosexhortosDAO();
                            $OfenfendidosexhortosDto->setIdOfenfendidoExhorto($testigoActuacion->getIdReferenciaParte());
                            $OfenfendidosexhortosDto = $OfenfendidosexhortosDao->selectOfenfendidosexhortos($OfenfendidosexhortosDto);
...
//                                           print_r($ActuacionesDto);
    ...
    public function generaCorreo($cedula, $nombrePersona, $fecha, $subject, $strCuerpoEmail) {
        ...
        // ****************** aqui llamar plantilla ******************************************************
        $objDatCorreo = $this->plantillaCorreo("notificacion");
        if ($objDatCorreo != "" && count($objDatCorreo) > 0) {
            //echo "objeto generado"
        }
        $subjectCorreo = $subject;
        // ***************** aqui fin de llamar plantilla ************************************************
        $mail = new EmailHELPER();
        $mail->setSenderAddress($objDatCorreo->CorreoRemite);
        //$mail->AddAddress("tmp.fernanda.ortega@pjedomex.gob.mx");
//          $mail->AddAddress("julio.moreno@mail3.pjedomex.gob.mx");//!!!!!!!
        $mail->setToAddress(trim($cedula . "@pjedomex.gob.mx"));
        $mail->setToName("Notificacion Electronica");
        $mail->setSubject($subjectCorreo);
        $mail->setHostSmtp($objDatCorreo->IpSMTP);
        $mail->setPortSmtp($objDatCorreo->PortSMTP);
        #AUTENTICACION
        $mail->setAuth($objDatCorreo->aut);
        $mail->setSmtpSecure($objDatCorreo->typeAut);
        $mail->setLoginUser($objDatCorreo->login);
        $mail->setLoginPass($objDatCorreo->password);
        #AUTENTICACION
        $mail->setIsHTMLFormat(true);
        # cuerpo del correo
        $mail->setBody($strCuerpoEmail);
