...
class JuzgadoresController {
    ...
    public function getJuzgadoresByCveJuzgado($JuzgadoresDto, $idSolicitudAudiencia) {
        try {
            ...
            if ($selectSolicitud == '')
                throw new Exception('No existe una solicitud de audiencia con el id = ' . $idSolicitudAudiencia);
            ...
            if (!isset($_SESSION['cveAdscripcion']))
                throw new Exception('No se inicio sesion correctamente.');
            ...
            if ($selectCatAudiencia == '')
                throw new Exception('no se encontro la categoria de la audiencia solicitada');
...
//        }
    ...
    public function consultarJuzgadoresMagistradosPorAdscripcion($juzgadoresDto, $cveJuzgado) {
        $selectDao = new SelectDAO();
        $param["fields"] = " j.* ";
        $param["tables"] = " tbljuzgadores j inner join tbltiposjuzgadores tj on (j.cveTipoJuzgador = tj.cveTipoJuzgador) inner join tbljuzgadoresjuzgados jj on (jj.idJuzgador = j.idJuzgador)  ";
        $param["conditions"] = " j.cveTipoJuzgador = " . $juzgadoresDto->getCveTipoJuzgador() . " and j.activo = 'S' and tj.activo = 'S'  and jj.activo='S'  and jj.cveJuzgado = " . $cveJuzgado . "  ";
    ...
    public function getJuzgadores($cveJuzgado = '') {
        ...
        $tmp = array();
        if ($cveJuzgado != '') {
            $juzgadoJuzgadosDto = new JuzgadoresjuzgadosDTO();
            $juzgadoJuzgadosDao = new JuzgadoresjuzgadosDAO();
            $juzgadoJuzgadosDto->setCveJuzgado($cveJuzgado);
            $juzgadoJuzgadosDto = $juzgadoJuzgadosDao->selectJuzgadoresjuzgados($juzgadoJuzgadosDto);
            if ($juzgadoJuzgadosDto != '') {
                $juzgadorDao = new JuzgadoresDAO();
                $contador = 0;
                foreach ($juzgadoJuzgadosDto as $jJ) {
                    $juzgadorDto = new JuzgadoresDTO();
                    $juzgadorDto->setIdJuzgador($jJ->getIdJuzgador());
                    $juzgadorDto = $juzgadorDao->selectJuzgadores($juzgadorDto);
                    if ($juzgadorDto != '') {
                        $juzgadores[$contador]['id'] = $juzgadorDto[0]->getIdJuzgador();
                        $juzgadores[$contador]['nombre'] = utf8_encode($juzgadorDto[0]->getNombre() . ' ' . $juzgadorDto[0]->getPaterno() . ' ' . $juzgadorDto[0]->getMaterno());
                        $juzgadores[$contador]['activo'] = $juzgadorDto[0]->getActivo();
                        $contador++;
                    }
                }
            }
            ...
            $juzgadorDto = new JuzgadoresDTO();
            $juzgadorDao = new JuzgadoresDAO();
            $juzgadorDto = $juzgadorDao->selectJuzgadores($juzgadorDto, ' ORDER BY nombre ASC ');
            if ($juzgadorDto != '') {
                $contador = 0;
                foreach ($juzgadorDto as $jJ) {
                    $juzgadores[$contador]['id'] = $jJ->getIdJuzgador();
                    $juzgadores[$contador]['nombre'] = utf8_encode($jJ->getNombre() . ' ' . $jJ->getPaterno() . ' ' . $jJ->getMaterno());
                    $juzgadores[$contador]['activo'] = $jJ->getActivo();
                    $contador++;
                }
            }
        }

        $juzgadores = $this->record_sort($juzgadores, 'nombre');
