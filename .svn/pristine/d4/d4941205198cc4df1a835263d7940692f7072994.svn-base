...
include_once(dirname(__FILE__)."/../../../controladores/sigejupe/tiposdocumentos/TiposdocumentosController.Class.php");
...
$TiposdocumentosDto->setextencion(strtoupper(str_ireplace("'","",trim(utf8_decode((phpversion()<=5.5) ? mb_strtoupper($TiposdocumentosDto->getextencion(),"utf8"):strtoupper($TiposdocumentosDto->getextencion()))))));
if($this->esFecha($TiposdocumentosDto->getextencion())){
$TiposdocumentosDto->setextencion($this->fechaMysql($TiposdocumentosDto->getextencion()));
}
...
$tiposdocumentosDto->setExtencion($extencion);
