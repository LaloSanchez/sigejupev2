...
//          error_log("Parametros ".print_r($AmparosDto,true));
        ...
        $AmparosDto = $AmparosController->selectAmparos($AmparosDto, "", $params);
...
//return $dtoToJson->toJson("RESULTADOS DE LA CONSULTA");
    ...
    public function insertAmparos($AmparosDto, $listaQuejosos, $listaTerceros, $tipoPersonaAdd) {
        $AmparosDto = $this->validarAmparos($AmparosDto);
        $AmparosController = new AmparosController();
        $AmparosDto = $AmparosController->insertAmparos($AmparosDto, "", $listaQuejosos, $listaTerceros, $tipoPersonaAdd);
    ...
    public function updateAmparos($AmparosDto, $listaQuejosos, $listaTerceros, $tipoPersonaAdd) {
        ...
        $AmparosDto = $this->validarAmparos($AmparosDto);
        $AmparosController = new AmparosController();
        $AmparosDto = $AmparosController->updateAmparos($AmparosDto, "", $listaQuejosos, $listaTerceros, $tipoPersonaAdd);
        if ($AmparosDto != "") {
            $dtoToJson = new DtoToJson($AmparosDto);
    ...
    public function deleteAmparos($AmparosDto) {
        $AmparosDto = $this->validarAmparos($AmparosDto);
        $AmparosController = new AmparosController();
        $AmparosDto = $AmparosController->deleteAmparos($AmparosDto);
        if ($AmparosDto == true) {
            $jsonDto = new Encode_JSON();
...
if (($accion == "guardar") && ($idAmparo == "")) {
    ...
    if($numero !="" && $anio != "" && $cveTipoCarpeta != "0"){
        ...
        if($carpetasJudicialesDto != ""){
            $amparosDto->setIdCarpetaJudicial($carpetasJudicialesDto[0]->getIdCarpetaJudicial());  
          $amparosDto = $amparosFacade->selectAmparos($amparosDto, $txtFecInicialBusqueda, $txtFecFinalBusqueda,$cantxPag);
        }else{
            $respuesta = array("datos" => "", "estatus" => "error", "totalRegistros" => 0);
            $amparosDto = json_encode($respuesta);
        }
