...
//                $datos["groups"] = "c.idCateo";
    ...
    public function reporteCateos($cateosDto, $datos, $paginacion) {
        $sqlIntervalo = "";
        $params = array();
        $params["fields"] = " COUNT(distinct(c.idCateo)) AS totalCount,";
        ...
        $params["fields"] .= "c.idCateo,c.anioCateo,c.numeroCateo,
            ...
            c.cveRespuestaSolicitudCateo,rsc.desRespuestaSolicitudCateo,
        ...
        $params["tables"] = "tblcateos c
            ...
            INNER JOIN tblrespuestasolicitudcateo rsc ON c.cveRespuestaSolicitudCateo = rsc.cveRespuestaSolicitudCateo
        ...
        if ($datos["cveEstatusSolicitudCateo"] != 0) {
            $params["conditions"].=" AND sc.cveEstatusSolicitudCateo=" . $datos["cveEstatusSolicitudCateo"];
        }
        if ($datos["cveRespuestaSolicitudCateo"] != 0) {
            $params["conditions"].=" AND c.cveRespuestaSolicitudCateo=" . $datos["cveRespuestaSolicitudCateo"];
        }
        ...
        if (($paginacion["paginacion"] == "false") || ($paginacion["paginacion"] == false)) {
            ...
            if ($datos["detalles"] == "detalle" || $datos["nivel"] == 5) {
                $aux = "count(distinct(x.idCateo)) as totalCount";
            }
