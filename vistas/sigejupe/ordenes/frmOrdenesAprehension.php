...
<script type="text/javascript">
    ...
    */
	...
	function disabledFields(stateKeyFields,stateModuleFields){
		...
		//Module fields
		idModuleFields = ['input_mcautelares_sintesis'];
                ...
                $.each(list, function(k, v){
                    ...
                    if (controlador){
                        if (v.imputadoCheck == false && v.imputadoActivoDiferenteActuacion != ''){
                            ...
                            name += '<span class="pull-right"><span class="glyphicon glyphicon-pushpin"></span>&nbsp;&nbsp;<span style="font-size:10pt;">Orden asignada el ' + v.imputadoActivoDiferenteActuacion + '</span></span>';
                            medidaImputado = true;
                        }
                    } else{
                        $.each(medidasAplicadas, function(llave, valor){
                            if (imputadoId == valor.idImputado){
                                name += '<span class="pull-right"><span class="glyphicon glyphicon-pushpin"></span>&nbsp;&nbsp;<span style="font-size:10pt;">Orden asignada el ' + valor.fecha + '</span></span>';
                                medidaImputado = true;
                            }
                        });
                    }
...
//                    }
                    tbody  +=  '			</h4>'
                        ...
                        + '				<label class="control-label col-xs-12 col-sm-2 col-md-2 needed">Tipo de orden</label>'
                        ...
                        + '				<table style="width:95%; margin-left:auto; margin-right:auto;" class="table table-responsive table-striped table-hover"><thead><tr><th colspan="2">Orden</th></tr></thead><tbody id="tablaMedidasCautelares_' + imputadoId +'"></tbody></table>'
                        + '			</div> <!-- lista de medidas cautelares/ -->'
                        + '			</div>'
                        + '			<!-- /seccion de datos para todas las medidas -->'
                        + '			<!-- seccion mensaje para ingresar al menos una medida cautelar  -->'
                ...
                fillCombo(tiposMedidas,'tiposordenes/TiposordenesFacade','cveTipoOrden','desTipoOrden');
            }else{
                //habilita campos
                disabledFields(false,true);
	...
	*/
	...
	function saveMcautelares(){
		...
		if(idActuacion == ''){
			//es insercion
			accion = 'guardaOrdenesAprehension';
		}else{
			//es actualizacion
			accion = 'actualizaOrdeneApehension';
		}
		...
		$.post("../fachadas/sigejupe/actuaciones/ActuacionesFacade.Class.php",
			{
				idActuacion:idActuacion,
				activo:activo,
				accion:'borraOrdenesAprehension'
			},function(response){
			var object = eval('(' + response + ')');
			var totalRecords = object.totalCount;
			var message = object.text;
	...
	*/
	function loadApelacionInfo(fullId){
		...
		var tablaInfo = '<table><tr><td style="text-align:right;">Apelaci&oacute;n de:</td>'
			...
			+ '<tr><td style="text-align:right;">Para la Orden:</td>'
		...
		$.post("../fachadas/sigejupe/actuaciones/ActuacionesFacade.Class.php",
			{
				...
				pag:page,
				cantxPag:cantReg,
				accion:'buscarOrdenesAprehension'
			},function(response){
				var object = eval('(' + response + ')');
				var totalRecords = object.totalCount;
				if(totalRecords>0){
	...
	*/
	function imputadosTableCheck(imputados,tipoCarpeta){
		...
		$.each(imputados, function(k,v){
			...
			comboEspecial('tiposordenes/TiposordenesFacade.Class.php','select_mcautelares_medidas_'+idImputadoCarpeta,v.cveTipoMedidaCautelar,'cveTipoOrden','desTipoOrden');
			//habilita el combo y boton de ediciOn
...
/*					if(localStorage.getItem("textoObservaciones") != ''){
						...
						console.log('uno');
					}else if(localStorage.contenidoDocumento!=''){
						value = localStorage.getItem("contenidoDocumento");
						localStorage.removeItem("contenidoDocumento");
						console.log('dos');
					}
*/					console.log('value');
...
//    }
	...
	*/
	...
	$("#btn_buscaCarpeta").on('click', function(){
		...
		var activo = 'S';
		var accion = 'buscarImputadosOrdenesAprehension';
		var idActuacion = idActuacion || '';
	...
	* Variables varias
	*/
	var titulos = {'titulo1':'Ordenes','titulo2':'B&uacute;squeda','titulo3':'Resultados','titulo4':'Captura de Apelaci&oacute;n'};
	var cveJuzgado = $('#SysCveAdscripcion').val();
	var cveUsuario = $('#SysCveUsuarioSistema').val();
	var cveTipoActuacion = 15;
	var imputadosMC = '';
    ...
    function visorDocuemntos() {
            $.ajax({
                ...
                data: {idCarpetaJudicial: "", idActuacion: $('#idActuacion').val(), cveTipoDocumento: 18}, //malo
                async: false,
                dataType: 'html',
                beforeSend: function () {
     ...
     function enviar(){
        var strDatos = "accion=generarJson";
            ...
            strDatos += "&cveTipoDocumento=18"; //tipo documento
