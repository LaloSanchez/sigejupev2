...
if (isset($_SESSION["cveUsuarioSistema"]) && $_SESSION["cveUsuarioSistema"] !== "") {
                ...
                <form id="formProgramaciones" method="post">
                    ...
                    <div class="form-group">
                        ...
                        <div class="col-xs-9" id="listaMeses">
                            <input type="text" class="form-control" id="desGrupoMuestra" name="desGrupoMuestra" placeholder="Descripcion" maxlength="100">
                            <input type="hidden" id="cveGrupoMuestra" name="cveGrupoMuestra" />
                        </div>
                    </div>
    ...
    <script type="text/javascript">
        ...
        consultaGrupo = function(Aux){
            ...
            var descripcion = $("#desGrupoMuestra").val();
            var html = "";
            $.ajax({
                type: "POST",
                    ...
                    try{
                        ...
                        if (result.totalCount > 0) {
                            ...
                            for (var n = 0; n < result.totalCount; n++){
                                c++;
                                html += '<tr>';
                                html += '<td onclick="consultaId(' + result.data[n].cveGrupoMuestra + ')">' + c + '</td>';
                                html += '<td onclick="consultaId(' + result.data[n].cveGrupoMuestra + ')">' + result.data[n].desGrupoMuestra + '</td>';
                                html += '<td onclick="consultaId(' + result.data[n].cveGrupoMuestra + ')">' + formatoFecha(result.data[n].fechaRegistro) + '</td>';
                                html += '<td onclick="consultaId(' + result.data[n].cveGrupoMuestra + ')">' + formatoFecha(result.data[n].fechaActualizacion) + '</td>';
                                html += '<td><input type="checkbox" name="eliminar[]" value="' + result.data[n].cveGrupoMuestra + '"></td>';
                                html += '</tr>';
                            }
                            html += '</tbody>';
                            html += '</table>';
        ...
        getPaginas = function (pag, cantReg) {
            ...
            strDatos += "&activo=S";
            strDatos += "&desGrupoMuestra=" + $("#desGrupoMuestra").val();
        ...
        mostrarJuzgados = function(cve){
            ...
            if (cve > 0){
                $.ajax({
                    type: "POST",
                    url: "../fachadas/sigejupe/gruposmuestrasjuzgados/GruposmuestrasjuzgadosFacade.Class.php",
                    data: {
                    cveGrupoMuestra : cve,
                    activo: "S",
                    accion : "consultar"},
        ...
        clean = function () {
            //$.clearInput = function () {                                    
                ...
                $("#divFormulario").find('input[name="cveGrupoMuestra"]').val("");
                $(".elimina").hide();
                var html = '';
                $("#consulta-grupos").html(html);
        ...
        seleccionaGrupo = function(cveGrupoMuestra){
            $.ajax({
                type: "POST",
                url: "../fachadas/sigejupe/gruposmuestras/GruposmuestrasFacade.Class.php",
                data: {
                ...
                success: function(datos) {
                    try {
                        if (datos.totalCount > 0) {
                            $("#cveGrupoMuestra").val(datos.data[0].cveGrupoMuestra);
                            $("#desGrupoMuestra").val(datos.data[0].desGrupoMuestra);
                            $("#listaHorarios").html("");
                            //ToggleLoading(2);
                            $(".elimina").show();
                            mostrarJuzgados(datos.data[0].cveGrupoMuestra);
                        }else{
                            //alert(datos.text);
                            $("#advertencia").html('<span>' + datos.text + '</span>').fadeIn('slow').delay(4000).fadeOut('slow');
                            $("#listaGrupos").html("");
        ...
        guardarGrupo = function(){
            var cveGrupoMuestra = $("#cveGrupoMuestra").val();
            var desGrupoMuestra = $("#desGrupoMuestra").val();
            ...
            if( validarCampos() ) {
                $.ajax({
                    ...
                    data: {
                        cveGrupoMuestra : cveGrupoMuestra,
                        desGrupoMuestra : desGrupoMuestra,
        ...
        function validarCampos(){
            ...
            if ($("#desGrupoMuestra").val() == "" ){
                $("#desGrupoMuestra").addClass("valida-error");
