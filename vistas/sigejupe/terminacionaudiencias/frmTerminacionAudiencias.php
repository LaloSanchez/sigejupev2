...
if (isset($_SESSION["cveUsuarioSistema"]) && $_SESSION["cveUsuarioSistema"] !== "") {
    ...
    <div class="panel panel-default">
        <div class="panel-heading">
            <h5 class="panel-title">                                                            
                TERMINACI&Oacute;N DE AUDIENCIAS
    ...
    //                    $('#cmbJuzgador').empty();
        ...
        cargaInfo = function () {
            ...
            var count = [];
            if (juzgado == 0) {
                datos = "accion=selectAudienciasHorariosTerminacion&fechaInicial=" + fecha + "&activo=S";
            } else {
                datos = "accion=selectAudienciasHorariosTerminacion&fechaInicial=" + fecha + "&cveJuzgado=" + juzgado + "&activo=S";
            }
            $.ajax({
                type: "POST",
        ...
        cargaInfoJuzgador = function () {
            ...
            var array = [];
            var count = [];
            var datos = "accion=selecAdmintAudienciasJuzgadorTerminacion&variable=" + juzgador + "&fechaInicial=" + fecha + "&activo=S";
            $.ajax({
                type: "POST",
                url: "../fachadas/sigejupe/audiencias/AudienciasFacade.Class.php",
                async: false,
    ...
    //                    setTimeAlert("divAlertWarningAudiencias");
        ...
        cargarInfoSalas = function () {
            ...
            var array = [];
            var count = [];
            var datos = "accion=selecAdmintAudienciasSalasTerminacion&cveSala=" + cveSala + "&fechaInicial=" + fecha + "&activo=S";
            $.ajax({
                type: "POST",
                url: "../fachadas/sigejupe/audiencias/AudienciasFacade.Class.php",
                async: false,
        ...
        valida = function () {
            ...
            if ($('#hddIdAudiencia').val() != "" || $('#hddIdAudiencia').val() != "0") {
                if ($('#cmbEstatus').val() == "" || $('#cmbEstatus').val() == "0") {
                    ...
                    if ($('#cmbEstatus').val() == "1") {
                        mensaje += "*El estatus de la audiencia debe ser diferente a 'Por celebrar' <br>";
                        error = false;
                    }
                }
            } else {
            ...
            if ($("#hddIdAudiencia").val() != "") {
                ...
                if (valida()) {
                    $.ajax({
                        ...
                        async: false,
                        dataType: "json",
                        data: {accion: "terminacionAudiencias",
    ...
    //                    comboSalas();
        ...
        imprimir = function () {
            if ($("#hddIdAudiencia").val() != "") {
                ...
                window.open("../fachadas/sigejupe/resumensolicitudaudiencia/ResumensolicitudaudienciaFacade.php?idSolicitud=" + idAudiencia + "&accion=consultaAcuseAudiencia", 'Reporte', '"scrollbars=1,width=800, height=1000');
            } else {
                $("#divAlertWarningAudiencias").html("");
    ...
    //        comboJuzgadores("cmbJuzgador3");
            ...
            $("#hddIdAudiencia").attr('disabled', 'disabled');

            $("#hddIdAudiencia").attr('disabled', 'disabled');
            $("#cmbTipoCarpeta").attr('disabled', 'disabled');
            $("#numero").attr('disabled', 'disabled');
            $("#anio").attr('disabled', 'disabled');
            $("#cmbJuzgadosDesahoga").attr('disabled', 'disabled');
            $("#cmbSala").attr('disabled', 'disabled');
            $("#txtFechaInicial").attr('disabled', 'disabled');
            $("#txtFechaFinal").attr('disabled', 'disabled');
            $("#cmbCatAudiencia").attr('disabled', 'disabled');
            $("#cmbJuzgador1").attr('disabled', 'disabled');
            $("#cmbJuzgador2").attr('disabled', 'disabled');
            $("#cmbJuzgador3").attr('disabled', 'disabled');
            $("#cmbFunciones1").attr('disabled', 'disabled');
            $("#cmbFunciones2").attr('disabled', 'disabled');
            $("#cmbFunciones3").attr('disabled', 'disabled');

            /////
            $("#hddIdJuzgadorAudiencia1").attr('disabled', 'disabled');
            $("#hddIdJuzgadorAudiencia2").attr('disabled', 'disabled');
            $("#hddIdJuzgadorAudiencia3").attr('disabled', 'disabled');
        });
