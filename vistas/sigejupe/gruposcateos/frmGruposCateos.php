...
if (isset($_SESSION["cveUsuarioSistema"]) && $_SESSION["cveUsuarioSistema"] !== "") {
                ...
                <form id="formProgramaciones" method="post">
                    ...
                    <div class="form-group">
                        ...
                        <div class="col-xs-9" id="listaMeses">
                            <input type="text" class="form-control" id="desGrupoCateo" name="desGrupoCateo" placeholder="Descripcion" maxlength="100">
                            <input type="hidden" id="cveGrupoCateo" name="cveGrupoCateo" />
                        </div>
                    </div>
    ...
    <script type="text/javascript">
        ...
        consultaGrupo = function(Aux){
            ...
            var descripcion = $("#desGrupoCateo").val();
            var html = "";
            $.ajax({
                type: "POST",
                    ...
                    try{
                        ...
                        if (result.totalCount > 0) {
                            ...
                            for (var n = 0; n < result.totalCount; n++){
                                c++;
                                html += '<tr>';
                                html += '<td onclick="consultaId(' + result.data[n].cveGrupoCateo + ')">' + c + '</td>';
                                html += '<td onclick="consultaId(' + result.data[n].cveGrupoCateo + ')">' + result.data[n].desGrupoCateo + '</td>';
                                html += '<td onclick="consultaId(' + result.data[n].cveGrupoCateo + ')">' + formatoFecha(result.data[n].fechaRegistro) + '</td>';
                                html += '<td onclick="consultaId(' + result.data[n].cveGrupoCateo + ')">' + formatoFecha(result.data[n].fechaActualizacion) + '</td>';
                                html += '<td><input type="checkbox" name="eliminar[]" value="' + result.data[n].cveGrupoCateo + '"></td>';
                                html += '</tr>';
                            }
                            html += '</tbody>';
                            html += '</table>';
        ...
        getPaginas = function (pag, cantReg) {
            ...
            strDatos += "&activo=S";
            strDatos += "&desGrupoCateo=" + $("#desGrupoCateo").val();
        ...
        mostrarJuzgados = function(cve){
            ...
            if (cve > 0){
                $.ajax({
                    type: "POST",
                    url: "../fachadas/sigejupe/gruposjuzgados/GruposjuzgadosFacade.Class.php",
                    data: {
                    cveGrupoCateo : cve,
                    activo: "S",
                    accion : "consultar"},
        ...
        clean = function () {
            //$.clearInput = function () {                                    
                ...
                $("#divFormulario").find('input[name="cveGrupoCateo"]').val("");
                $(".elimina").hide();
                var html = '';
                $("#consulta-grupos").html(html);
        ...
        seleccionaGrupo = function(cveGrupoCateo){
            $.ajax({
                type: "POST",
                url: "../fachadas/sigejupe/gruposcateos/GruposcateosFacade.Class.php",
                data: {
                ...
                success: function(datos) {
                    try {
                        if (datos.totalCount > 0) {
                            $("#cveGrupoCateo").val(datos.data[0].cveGrupoCateo);
                            $("#desGrupoCateo").val(datos.data[0].desGrupoCateo);
                            $("#listaHorarios").html("");
                            //ToggleLoading(2);
                            $(".elimina").show();
                            mostrarJuzgados(datos.data[0].cveGrupoCateo);
                        }else{
                            //alert(datos.text);
                            $("#advertencia").html('<span>' + datos.text + '</span>').fadeIn('slow').delay(4000).fadeOut('slow');
                            $("#listaGrupos").html("");
        ...
        guardarGrupo = function(){
            var cveGrupoCateo = $("#cveGrupoCateo").val();
            var desGrupoCateo = $("#desGrupoCateo").val();
        ...
        function validarCampos(){
            ...
            if ($("#desGrupoCateo").val() == "" ){
                $("#desGrupoCateo").addClass("valida-error");
