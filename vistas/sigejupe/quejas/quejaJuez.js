var quejaJuez = function(){
	return {
        ...
        buscarQuejaInterpuesta:function(){
            var self = this;
            var numEmpleado = $('#hiddenNumEmpleado').val();
            ...
            var data = "numEmpleado=" + numEmpleado + "&cveJuzgado=" + cveTipoJuzgado_busqueda + "&cveTipoCarpeta=" + cmbTipoCarpeta_busqueda + "&numero=" + txtNumero_busqueda + "&anio=" + txtAnio_busqueda + "&numActuacion=" + txtNumeroActuacion_busqueda + "&aniActuacion=" + txtAnioActuacion_busqueda +"&fechaDesde=" + txtFechaInicial_busqueda + "&fechaHasta=" + txtFechaFinal_busqueda + "&paginacion=true&cantxPag=10&pag=1&accion=buscaQuejasInterpuestasJuez";
        ...
        muestraRegistro:function(posicion){
            ...
            this.actualizaEstadoQueja(rg.idActuacion, rg.idActuacionesEstatus, 36); //idActuacion, cveEstatus -tabla de estados-
            ...
            if( rg.cveEstatus >= 37 && rg.cveEstatus <=41 ){
                $('#btnGuardaSubsana').prop('disabled',true);
                $('#btnGuardaConsejo').prop('disabled',true);
            ...
            var tabla = '<table style="margin-left:auto; margin-right: auto; width: 90%;" class="table table-hover table-bordered">'
                ...
                llenareditor(rg.acuerdo);
                //muestra resolucion del consejo
        ...
        limpiarFormulario:function(seccion){
        	switch( seccion ){
                ...
                case 'acuerdo':
                    ...
                    $('#hiddenIdQuejaPromocion').val('');
                    $('#hiddenIdActuacionesEstatus').val('');
                    $('#btnGuardaSubsana').prop('disabled',false);
                    $('#btnGuardaConsejo').prop('disabled',false);
        ...
        guardaSubsana:function(){
            ...
            var strDatos = "accion=guardar&idQuejaProm=" + idQuejaProm + "&acuerdo=" + textoAcuerdo + "&fechaAcuerdo=" + fechaHoy;
            var self = this;
            //guarda el Acuerdo
            $.ajax({
                type: "POST",
                ...
                success: function (respuesta) {
                    ...
                    if( objeto.totalCount > 0 ){
                        //Subsana
                        self.actualizaEstadoQueja( idActuacion, idActuacionesEstatus, 38 ); //38, resuelta por el juez
                        $('#btnGuardaSubsana').prop('disabled',true);
                        $('#btnGuardaConsejo').prop('disabled',true);
                        $('#btnLimpiarAcuerdo').prop('disabled',true);
        ...
        guardaConsejo:function(){
            ...
            $.ajax({
                ...
                success: function (respuesta) {
                    ...
                    if( objeto.totalCount > 0 ){
                        //Subsana
                        self.actualizaEstadoQueja( idActuacion, idActuacionesEstatus, 37 ); //37, acordada por el juez
                        $('#btnGuardaSubsana').prop('disabled',true);
                        $('#btnGuardaConsejo').prop('disabled',true);
                        $('#btnLimpiarAcuerdo').prop('disabled',true);
                        muestraMensaje('Acuerdo Guardado correctamente.','success');
