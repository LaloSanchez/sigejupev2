...
if (isset($_SESSION["cveUsuarioSistema"]) && $_SESSION["cveUsuarioSistema"] !== "") {
    $type = 1; //--> General
    $title = "Consulta de Solicitudes de Cateo Administrador General";
    if (isset($_GET['tipoConsulta'])) {
        switch ($_GET['tipoConsulta']) {
            case "mp":
                $type = 2;
                $title = "Consulta de Solicitudes de Cateo M.P.";
                break;
            case "juzgador":
                $title = "Consulta de Solicitudes de Cateo Juzgador";
                $type = 3;
                break;
            case "adminJuzgado" :
                $title = "Consulta de Solicitudes de Cateo Administrador Juzgado";
                $type = 4;
                break;
            case "secretario" :
                $title = "Consulta de Solicitudes de Cateo Secretario";
                $type = 6;
                break;
        }
    }
    $idCateo = (isset($_POST["idCateo"]) ? $_POST["idCateo"] : "");
    ?>
