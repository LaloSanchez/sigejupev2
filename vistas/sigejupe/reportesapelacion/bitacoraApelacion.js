var BitacoraApelacion = function () {
    return {
        idCateo: 0,
        /////////// FUNCIONES PARA ADMINISTRACION DE APELACION CATEOS
        searchCateo: function (pagAux) {
            ...
            var datos = {
                ...
                "anio": anio,
                "type": $("#type").val(),
                "accion": "consultarApelacionInformacionBitacora",
                "pag": pag,
                "cantxPag": cantReg
            }
            $(".msj").hide();
                    ...
                    if (datos.status == "OK") {
                        ...
                        if (datos.datos !== "undefined") {
                            $.each(datos.datos, function (index, val) {
                                table += "<tr onclick='javascript:bitacora.muestraInfo(" + val.idCateos + "," + val.cveEstatusCateo + ")' >";
                                table += "<td>" + (i + 1) + "</td>";
        ...
        muestraInfo: function (idCateo, cveEstatusCateo) {
            if (cveEstatusCateo == "7" || cveEstatusCateo == "8") {
                var self = this;
                $.ajax({
                    url: "../fachadas/sigejupe/apelacioncateos/ApelacioncateosFacade.Class.php",
                    data: "accion=consultarDetalleCateoApelacion&idCateo=" + idCateo,
                    type: "POST",
                    success: function (datos) {
                        try {
                            datos = eval("(" + datos + ")");
            ...
            } else if (cveEstatusCateo == "9" || cveEstatusCateo == "10") {
                $("#divAlertWarning").html("");
                $("#divAlertWarning").html("Solicitud de Apelaci&oacute;n Contestado Por Juez. No se puede realizar modificaciones");
                $("#divAlertWarning").show("slide");
                setTimeAlert("divAlertWarning");
            } else if (cveEstatusCateo == "11") {
                $("#divAlertWarning").html("");
                $("#divAlertWarning").html("Solciitud de Apelaci&oacute;n Contestado Por Secretario. No se puede realizar modificaciones");
                $("#divAlertWarning").show("slide");
                setTimeAlert("divAlertWarning");
            }
        },
        ...
        sendInfo: function (idCateo, idJuez) {
            ...
            var type = $("#type").val();
            var datos = {
                "accion": "actualizarJuzgadorApelacion",
                "idCateo": idCateo,
                "idJuzgador": idJuez,
                "type": type
            };
