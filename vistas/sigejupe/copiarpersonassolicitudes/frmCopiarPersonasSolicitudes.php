...
if (isset($_SESSION["cveUsuarioSistema"]) && $_SESSION["cveUsuarioSistema"] !== "") {
    ...
    <div class="panel panel-default">
        <div class="panel-heading">
            <h5 class="panel-title">
                Copiar Personas de una Causa a una Solicitud
            </h5>
        </div>
        ...
        <div class="panel-body">
            <div class="form-horizontal" id="divFormulario">
                ...
                <div style="width: 50%; float: left;">
                    ...
                    <input type="hidden" id="idSolicitudAudiencia" name="idSolicitudAudiencia">
                ...
                <div style="width: 50%; float: left;">
                    ...
                    <div id="listaPersonasDestino" style="padding: 5px; display: none;"></div>
                    <br><br>
                </div>
    ...
    <script type="text/javascript">
        ...
        busquedaDestino = function () {
            if($("#cmbJuzgadoDestino").val() == ""){
                ...
                TotalOfendidos = [];
                $("#guardar").hide();
                $("#listaPersonasDestino").html("");
                $("#listaPersonasDestino").hide();
                $.ajax({
                    type: "POST",
                    url: "../fachadas/sigejupe/carpetasjudiciales/CarpetasjudicialesFacade.Class.php",
                    data: {
                    cveJuzgadoAudiencia : $("#cmbJuzgadoDestino").val(),
                    numeroAudiencia: $("#txtNumeroDestino").val(),
                    anioAudiencia: $("#txtAnioDestino").val(),
                    cveTipoCarpetaAudiencia: $("#cmbTipoCarpetaDestino").val(),
                    tipoPersonaAudiencia: $("#cmbTipoParteDestino").val(),
                    activo: "S",
                    accion : "consultarAudiencias"},
                    async: true,
                    dataType: "html",
                    ...
                    success: function(datos){
                        try{
                            ...
                            if(result.totalCount > 0){
                                ...
                                html += '<div style="text-align: center;">Audiencias encontradas</div>';
                                html += '<br>';
                                html += '<table id="resultAudiencias" class="table table-hover table-striped table-bordered">';
                                html += '<thead>';
                                html += '<th>#</th>';
                                ...
                                for( var n = 0; n < result.totalCount; n++ ){
                                    ...
                                    if ( $("#cmbTipoParteDestino").val() == "1" ) {
                                        html += '<tr class="filaDestino" onClick="consultarImputadosSolicitud(' + result.data[n].idSolicitudAudiencia + ')">';
                                    } else if( $("#cmbTipoParteDestino").val() == "2" ) {
                                        html += '<tr class="filaDestino" onClick="consultarOfendidosSolicitud(' + result.data[n].idSolicitudAudiencia + ')">';
                                    }
                                    
                                    html += '<td>' + c + '</td>';
                                    html += '<td>' + result.data[n].desCatAudiencia + '</td>';
                                    html += '<td>' + result.data[n].desTipoCarpeta + ' ' + result.data[n].numero + '/' + result.data[n].anio + '</td>';
                                    html += '<td>' + result.data[n].desJuzgado + '</td>';
                                    html += '<td>' + result.data[n].desSala + '</td>';
                                    html += '<td>' + result.data[n].desEstatusAudiencia + '</td>';
                                    html += '</tr>';
                                }
                                html += '</tbody>';
                                html += '</table>';
                                $("#resultadosDestino").html(html);
                                $("#resultAudiencias").DataTable({
                                    paging: false,
                                    searching: false
                                });
                            } else{
                                $("#div-advertencia").html('<span>' + result.mnj + '</span>').fadeIn('slow').delay(4000).fadeOut('slow');
                                html += '';
                                $("#resultadosDestino").html(html);
    ...
    //                    $(".consulta").show();
        ...
        consultarImputadosSolicitud = function(idSolicitudAudiencia){
            arrayImputadosDestino = [];
            arrayOfendidosDestino = [];
            $("#hddImputados").val("");
            $("#hddOfendidos").val("");
            ...
            TotalOfendidos = [];
            $("#divResultadosImputados").html("");
            $("#idSolicitudAudiencia").val(idSolicitudAudiencia);
            $("#listaPersonasDestino").html("");
            $("#listaPersonasDestino").hide();
            $.ajax({
                type: "POST",
                ...
                success: function (datos) {
                    ...
                    if (datos.status == 'Ok') {
                        var table = "<div><input type='button' class='btn btn-primary' onClick='mostrarTablaAudiencias()' value='Regresar'></div>";
                        table += '<br>';
                        table += '<table id="tablaResultadosDestino" class="table table-hover table-striped table-bordered">';
                        table += '<head>';
                        table += '<th width="40%">Tipo de persona</th>';
                        table += '<th width="40%" >Nombre</th>';
                        table += '<th width="40%" >G\u00e9nero</th>';
                        table += '<th width="10%">&nbsp;</th></thead>';
                        table += '<tbody>';
                        for (var i = 0; i < datos.totalCount; i++) {
                            table += '<tr><td id="' + datos.data[i].idImputadoSolicitud + '" >' + datos.data[i].desTipoPersona + "</td>";
                            if (datos.data[i].cveTipoPersona == '1') {
                                table += '<td>' + datos.data[i].nombre + " " + datos.data[i].paterno + " " + datos.data[i].materno + '</th>';
                            } else {
                            ...
                            table += "<td><input type='radio' name='idImputadoDestino' value='" + datos.data[i].idImputadoSolicitud + "'></td></tr>";
                            
                            arrayImputadosDestino[i] = datos.data[i];
                        }
                        table += '</tbody>';
                        table += '</table>';
                        $('#listaPersonasDestino').html(table);
                        $('#listaPersonasDestino').show();
                        $("#resultadosDestino").hide();
    //                    $("#tablaResultadosDestino").DataTable({
    //                        paging: false,
    //                        searching: false
    //                    });
                    }
        ...
        consultarOfendidosSolicitud = function(idSolicitudAudiencia){
            $("#idSolicitudAudiencia").val(idSolicitudAudiencia);
            arrayImputadosDestino = [];
            arrayOfendidosDestino = [];
            $("#hddImputados").val("");
                ...
                success: function (data) {
                    try {
                        if (data.totalCount > 0) {
                            var html = "";
                            html = "<div><input type='button' class='btn btn-primary' onClick='mostrarTablaAudiencias()' value='Regresar'></div>";
                            html += '<div>Sujetos pasivos del Delito</div>';
                            html += '<br>';
                            html += '<table id="tablaResultadosDestino" class="table table-hover table-striped table-bordered">';
                            html += '<thead>';
                            html += '<th width="40%">Tipo de persona</th>';
                            html += '<th width="40%">Nombre</th>';
                            html += '<th width="40%">G\u00e9nero</th>';
                            html += '<th width="10%">&nbsp;</th></thead>';
                            html += '<tbody>';
                            for ( var n = 0; n < data.totalCount; n++ ) {
                                var success = '';//($("#idOfendidoSolicitud").val() == e.idOfendidoSolicitud) ? 'success' : '';
                               
                                html += "<tr data-ofendido='" + data.data[n].idOfendidoSolicitud + "' class='trSeleccion " + success + "'>";
                                html += "<td>" + data.data[n].desTipoPersona + "</td>";
                                if (data.data[n].cveTipoPersona == 1) {
                                    html += "<td>" + data.data[n].nombre + " " + data.data[n].paterno + " " + data.data[n].materno + "</td>";
                                } else if (data.data[n].cveTipoPersona == 2 || data.data[n].cveTipoPersona == 3) {
                                    html += "<td>" + data.data[n].nombreMoral + "</td>";
                                }
                                html += "<td>" + data.data[n].desGenero + "</td>";
                                html += "<td><input type='radio' name='idOfendidoDestino' value='" + data.data[n].idOfendidoSolicitud + "'></td>";

                                html += "</tr>";
                                arrayOfendidosDestino[n] = data.data[n];
                            }
                            html += '</tbody>';
                            html += "</table>";
                            $('#listaPersonasDestino').html(html);
                            $('#listaPersonasDestino').show();
                            $("#resultadosDestino").hide();
    //                        $("#tablaResultadosDestino").DataTable({
    //                            paging: false,
    ...
    //                        });
        ...
        mostrarTablaAudiencias = function() {
            $('#listaPersonasDestino').hide();
            $("#resultadosDestino").show();
        };
        
        cleanStepOneConsult = function() {
            $("#idSolicitudAudiencia").val("");
                            ...
                            if(result.totalCount > 0){
                                ...
                                for( var n = 0; n < result.totalCount; n++ ){
                                    ...
                                    if ( result.data[n].tipoPersona == "1" ) {
                                        ...
                                        var etapa = result.data[n].cveEtapaProcesal;
                                        switch(etapa) {
                                            case '1': html += '<td>ETAPA DE INVESTIGACION O PRELIMINAR</td>';
                                                break;
                                            case '2': html += '<td>ETAPA INTERMEDIA O PREPARACION DE JUICIO</td>';
                                                break;
                                            case '3': html += '<td>ETAPA DE JUICIO O DEBATE</td>';
                                                break;
                                            case '4': html += '<td>ETAPA DE EJECUCION DE SENTENCIA</td>';
                                        }
                                    }
        ...
        agregarImputados = function () {
            ...
            if (validateRelacion()) {
                ...
                if ( $("#cmbTipoParteOrigen").val() == "1" ) {
                    ...
                    var destino = ( $('input:radio[name=idImputadoDestino]').is(":checked") ) ? $('input:radio[name=idImputadoDestino]:checked').val() : 0;
                    ListaImputados = {
                    ...
                    $('input:radio[name=idImputadoDestino]').attr('checked', false);
                } else {
                    var ListaOfendidos = new Array();
                    var origen = $('input:radio[name=idOfendidoOrigen]:checked').val();
                    var destino = ( $('input:radio[name=idOfendidoDestino]').is(":checked") ) ? $('input:radio[name=idOfendidoDestino]:checked').val() : 0;
                    ListaOfendidos = {
                    ...
                    $('input:radio[name=idOfendidoDestino]').attr('checked', false);
                }
                    ...
                    for (var x = 0; x < TotalImputados.length; x++) {
                        ...
                        for (var r = 0; r < arrayImputadosDestino.length; r++) {
                            if ( TotalImputados[x]["idImputadoDestino"] == 0 ) {
                                ...
                                if (TotalImputados[x]["idImputadoDestino"] == arrayImputadosDestino[r]["idImputadoSolicitud"]) {
                                    if (arrayImputadosDestino[r]["cveTipoPersona"] == "1") {
                    ...
                    for (var x = 0; x < TotalOfendidos.length; x++) {
                        ...
                        for (var r = 0; r < arrayOfendidosDestino.length; r++) {
                            if ( TotalOfendidos[x]["idOfendidoDestino"] == 0 ) {
                                ...
                                if (TotalOfendidos[x]["idOfendidoDestino"] == arrayOfendidosDestino[r]["idOfendidoSolicitud"]) {
    ...
    //        }
            ...
            if ( $("#idSolicitudAudiencia").val() == "" ) {
                mensaje += "*Debe consultar una solicitud de audiencia para realizar la copia de datos\n";
                error = false;
            }
            if ( $("#cmbTipoParteOrigen").val() == "1" ) {
        ...
        //        }
                ...
                for (var i = 0; i < TotalImputados.length; i++) {
                    if ((TotalImputados[i]["idImputadoOrigen"] == $('input:radio[name=idImputadoOrigen]:checked').val()) && (TotalImputados[i]["idImputadoDestino"] == $('input:radio[name=idImputadoDestino]:checked').val())) {
                ...
                for (var i = 0; i < TotalImputados.length; i++) {
                    if ((TotalImputados[i]["idImputadoDestino"] == $('input:radio[name=idImputadoDestino]:checked').val())) {
        ...
        //        }
                ...
                for (var i = 0; i < TotalOfendidos.length; i++) {
                    if ((TotalOfendidos[i]["idOfendidoDestino"] == $('input:radio[name=idOfendidoDestino]:checked').val())) {
                ...
                bootbox.dialog({
                    ...
                    buttons: {
                        danger: {
                            ...
                            callback: function () {
                                var url = "";
                                var strDatos = "accion=copiarDatosSolicitud";
                                if ( $("#cmbTipoParteOrigen").val() == "1" ) {
                                ...
                                strDatos += "&idCarpetaJudicial=" + $("#idCarpetaJudicial").val();
                                strDatos += "&idSolicitudAudiencia=" + $("#idSolicitudAudiencia").val();
                                $.ajax({
                                    type: "POST",
                                    url: url,
                                    data: strDatos,
                                    ...
                                    success: function (datos) {
                                        try {
                                            ...
                                            if (result.estatus == "ok") {
                                                $("#div-correcto").html("Datos guardados correctamente, verifique si todos los datos de imputados u ofendidos deben corresponder a la Solicitud donde se copiaron los datos, para ello ingrese al men&uacute; ADMON JUZGADO / ADMINISTRACI&Oacute;N DE SOLICITUDES").fadeIn('slow').delay(7000).fadeOut('slow');
                                                $("#guardar").hide();
        ...
        function listaJuzgados(){
            var tipoJuzgado = $("#cmbJuzgadoOrigen :selected").attr("data-tipojuzgadobusqueda");
                    ...
                    try {
                        ...
                        if (data.totalCount > 0) {
                            ...
                            for (var index = 0; index < data.totalCount; index++) {
                                if ( data.data[index].cveTipojuzgado == tipoJuzgado ) {
    ...
    //                            }
        ...
        function validar(){
            ...
            if($("#idCarpetaJudicial").val() == "" || $("#idCarpetaJudicial").val() == "0"){
                mensaje += '*Debe consultar la carpeta judicial origen para copiar los datos\n';
                error = true;
            }
            if($("#idSolicitudAudiencia").val() == "" || $("#idSolicitudAudiencia").val() == 0){
    ...
    //        }
        ...
        $(document).ready(function(){
            ...
            var permisos = obtenerPermisosFormulario("HERRAMIENTAS", "COPIAR PERSONAS  A SOLICITUD DE AUDIENCIA");
