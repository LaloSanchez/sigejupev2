var misSolicitudes = function () {
    return {
                        ...
                        $.each(datos, function (index, val) {
                            if (index != "type" && index != "total" && index != "paginas") {
                                $.each(val, function (indexS, valS) {
                                    ...
                                    table += "<td>" + valS.solicitudCateo.nip + "</td>";
        ...
        sendInfo: function (datos) {
            ...
            $.ajax({
                type: "POST",
                url: "../fachadas/sigejupe/cateos/CateosFacade.Class.php",
                data: datos,
                async: true,
                dataType: "html",
...
//            var mm = date.getMinutes();
        ...
        getPersonas: function (registro, response) {
            var table = "<tbody>";
            $.each(registro, function (index, value) {
                ...
                if (value.cveOrigen == 2 && response == 1) {
                    table += "<tr>";
                    table += "<td>" + value.nombre + " " + value.paterno + " " + value.materno + "</td>";
                    table += "<td>" + value.domicilio + "</td>";
                    $("#cmbGeneroPersona").val(value.cveGenero);
            ...
            table += "</tbody>";
            if (response == 1) {
                $("#tblPersonasR").append(table);
            } else {
                $("#tblPersonas").append(table);
            }
        },
        getObjetos: function (registro, response) {
            var table = "<tbody>";
            $.each(registro, function (index, value) {
                if (value.cveOrigen == 1) {
                    table += "<tr>";
                    ...
                    if (data.type == "OK") {
                        ...
                        $.each(data.data, function (index, value) {
                            ...
                            if (value.documento != "") {
                                ...
                                $("fileFound").html("<div class='text-center' ><br /><a target='_blank' href='../" + pathDocument + "' >ARCHIVO PROPORCIONADO POR EL MINISTERIO P&Uacute;BLICO PARA LA SOLICITUD DE CATEO</a><br /><br /></div>");
                            } else {
            ...
            $.ajax({
                ...
                success: function (datos) {
                    ...
                    if (datos.type == "OK") {
                        ...
                        $("#divAlertSuccess").show("slide");
                        setTimeAlert("divAlertSuccess");
                        window.location.href = '../fachadas/sigejupe/cateos/CateosFacade.Class.php?action=descargaRespuestaCateo&idCateo=' + self.cveRegistro;
                        anterior6.style.display = "inline-block";
                        nueva.style.display = "inline-block";
                        imprimir.style.display = "inline-block";
        ...
        imprimirResolucionSecretario: function () {
            var self = this;
            $.ajax({
                type: "POST",
                url: "../fachadas/sigejupe/cateos/CateosFacade.Class.php",
                data: "accion=descargaResolucionSecretario&idCateo=" + this.cveRegistro,
                async: true,
                dataType: "html",
                beforeSend: function () {
                    $(".LoadInfo").html("Cargando Porfavor Espere...").show();
                ...
                success: function (datos) {
                    ...
                    if (datos.type == "OK") {
                        ...
                        $("#divAlertSuccess").show("slide");
                        setTimeAlert("divAlertSuccess");
                        window.location.href = '../fachadas/sigejupe/cateos/CateosFacade.Class.php?action=descargaResolucionSecretario&idCateo=' + self.cveRegistro;
                    } else {
                        $("#divAlertWarning").html("");
                        $("#divAlertWarning").html(datos.text);
                        $("#divAlertWarning").show("slide");
