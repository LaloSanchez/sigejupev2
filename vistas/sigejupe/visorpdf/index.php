...
/**
 * función que manda error a nivel navegador
...
/**
 * función que valida que una variable sea entero positivo
...
if(isset($_POST['idActuacion']) && !empty($_POST['idActuacion'])) 
{
    validaInt($_POST['idActuacion']);
}
elseif(isset($_POST['idCarpetaJudicial']) && !empty($_POST['idCarpetaJudicial'])) 
{
    validaInt($_POST['idCarpetaJudicial']);
}
else
{
    error('No hay informaci&oacute;n para procesar.');
}
...
include_once '../../../fachadas/sigejupe/imagenes/ImagenesFacade.Class.php';
include_once dirname(__FILE__) . '/../../../tribunal/host/Host.Class.php' ;
$imagenesFacade = new ImagenesFacade();
$json = $imagenesFacade->getImagenes($_POST['idCarpetaJudicial'], $_POST['idActuacion'], $_POST['cveTipoDocumento']);

$visor = new Host('../../../tribunal/host/config.xml', 'VISORDOCUMENTOSSIGEJUPE');
...
//                ventimp.close();
            ...
            printDivIE = function (divID) {
                ...
                w.document.write("<html><head><title>Poder Judicial del Estado de MÃ©xico</title></head><body>");
...
//    }
            ...
            Navegar = function (Opc) {
                ...
                $('divFoja').html("");
                muestraDoc(arrFoja[Indice].idImagen, Extension(arrFoja[Indice].ruta), arrFoja[Indice].ruta, arrFoja[Indice].idDocumentoImg, arrFoja[Indice].DatosCarpetajudicial.cveOficialia, json.tipoDocumento.cveTipoActuacion, json.tipoDocumento.principal);
            };
            ...
            generar = function () {
                ...
                if (json.status == "fail") {
                    ocultaBotones();
                    $('#divFoja').html("<span><b>" + json.mje + "</b></span>");
                } else {
                    if (json.totalCount > 0) {
                        ...
                        $('divFoja').html("<span class=\"Arial14\"><b>No hay documentos para esta secciÃ³n del expediente!</b></span>");
                    }
                }
...
/*
            actualizarArchivos = function () {
                ...
                var json = eval(GlobalJson);
                if (tipoConsulta == "2") {
                    alert("Opcion no permitida para usuarios Externos");
                } else {
                    if (json.data.type == "Error") {
                        ...
                        ocultaBotones();
                        var idUsuarioExterno = <?php echo $idUsuarioExterno; ?>;
                        var idCarpetaJudicial = <?php echo $idCarpetaJudicial; ?>;
                        var idActuacion = <?php echo $idActuacion; ?>;
   ...
   /*
            guardarcambios = function () {
                ...
                if (json != "") {
                    var idCarpetaJudicial = <?php echo $idCarpetaJudicial; ?>;
                    var idActuacion = <?php echo $idActuacion; ?>;
                    var idAudiencia = <?php echo $idAudiencia; ?>;
                    $.ajax({
                        type: "POST",
                        url: "../../controller/imagenes/ImagenesController.Class.php",
                        data: {action: "actualizaOrden", datosImagenes: json, idCarpetaJudicial: idCarpetaJudicial, idActuacion: idActuacion, idAudiencia: idAudiencia},
                        async: false,
                        dataType: "html",
                ...
                if (idImagenesBorrar != "") {
                    if (confirm("Esta seguro de eliminar las imagenes seleccionadas?")) {
                        ...
                        $.ajax({
                            type: "POST",
                            url: "../../controller/imagenes/ImagenesController.Class.php",
                            data: {action: "borrarImagenes", idImagenesBorrar: idImagenesBorrar, idCarpetaJudicial: idCarpetaJudicial, idActuacion: idActuacion, idAudiencia: idAudiencia},
                            async: false,
                            dataType: "html",
                            beforeSend: function (objeto) {
