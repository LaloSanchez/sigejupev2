...
// ARCHIVOS PARA insertar Testigos
include_once(dirname(__FILE__) . "/../../../modelos/sigejupe/dto/testigoscarpetasjudiciales/TestigoscarpetasjudicialesDTO.Class.php");
include_once(dirname(__FILE__) . "/../../../modelos/sigejupe/dao/testigoscarpetasjudiciales/TestigoscarpetasjudicialesDAO.Class.php");
...
//        var_dump($param);
        ...
        if ($ExhortosDto != "") {
            ...
            foreach ($ExhortosDto as $key => $exhorto) {
                ...
                $listaTestigosI = array();
                $listaTestigosO = array();
                ...
                if ($ImputadosexhortosDto != '') {
                    ...
                    foreach ($ImputadosexhortosDto as $imputado) {
                        ...
                        $TestigoscarpetasjudicialesDAO = new TestigoscarpetasjudicialesDAO();
                        $TestigoscarpetasjudicialesDTO = new TestigoscarpetasjudicialesDtO();
                        $TestigoscarpetasjudicialesDTO->setIdReferenciaParte($imputado->getidImputadoExhorto());
                        $TestigoscarpetasjudicialesDTO->setIdCarpetaJudicial($imputado->getIdExhorto());
                        $TestigoscarpetasjudicialesDTO->setActivo("S");

                        $TestigoscarpetasjudicialesDTO = $TestigoscarpetasjudicialesDAO->selectTestigoscarpetasjudiciales($TestigoscarpetasjudicialesDTO);
                        if($TestigoscarpetasjudicialesDTO != ""){
                            foreach ($TestigoscarpetasjudicialesDTO as $testigosI) {
                                if($imputado->getcveTipoPersona() == 1){
                                    $nombreImputado = $imputado->getNombre()." ".$imputado->getpaterno()." ".$imputado->getmaterno();
                                }else{
                                    $nombreImputado = $imputado->getnombreMoral(); 
                                }
                                $nombreTipoParte = $this->getTipoParte($testigosI->getCveTipoParte());

                                $resultadoTestigoI = array(
                                    "idTestigoCarpetaJudicial" => $testigosI->getIdTestigoCarpetaJudicial(),
                                    "idCarpetaJudicial" => $testigosI->getIdCarpetaJudicial(),
                                    "cveTipoCarpeta" => $testigosI->getCveTipoCarpeta(),
                                    "cveReferenciaTipoParte" => $testigosI->getCveReferenciaTipoParte(),
                                    "idReferenciaParte" => $testigosI->getIdReferenciaParte(),
                                    "cveTipoParte" => $testigosI->getCveTipoParte(),
                                    "nombreTipoParte" => $nombreTipoParte,
                                    "nombreTestigo" => $testigosI->getNombreTestigo(),
                                    "nombreImputado" => $nombreImputado
                                );
                            array_push($listaTestigosI, $resultadoTestigoI);
                            }
                        }
                ...
                if ($OfenfendidosexhortosDto != '') {
                    ...
                    foreach ($OfenfendidosexhortosDto as $ofendido) {
                        ...
                        $TestigoscarpetasjudicialesDAO = new TestigoscarpetasjudicialesDAO();
                        $TestigoscarpetasjudicialesDTO = new TestigoscarpetasjudicialesDtO();
                        $TestigoscarpetasjudicialesDTO->setIdReferenciaParte($ofendido->getIdOfenfendidoExhorto());
                        $TestigoscarpetasjudicialesDTO->setIdCarpetaJudicial($ofendido->getIdExhorto());
                        $TestigoscarpetasjudicialesDTO->setActivo("S");

                        $TestigoscarpetasjudicialesDTO = $TestigoscarpetasjudicialesDAO->selectTestigoscarpetasjudiciales($TestigoscarpetasjudicialesDTO);
                        if($TestigoscarpetasjudicialesDTO != ""){
                            foreach ($TestigoscarpetasjudicialesDTO as $testigosO) {
                                if($ofendido->getcveTipoPersona() == 1){
                                    $nombreOfendido = $ofendido->getNombre()." ".$ofendido->getpaterno()." ".$ofendido->getmaterno();
                                }else{
                                    $nombreOfendido = $ofendido->getnombreMoral(); 
                                }
                                $nombreTipoParte = $this->getTipoParte($testigosO->getCveTipoParte());

                                $resultadoTestigoO = array(
                                    "idTestigoCarpetaJudicial" => $testigosO->getIdTestigoCarpetaJudicial(),
                                    "idCarpetaJudicial" => $testigosO->getIdCarpetaJudicial(),
                                    "cveTipoCarpeta" => $testigosO->getCveTipoCarpeta(),
                                    "cveReferenciaTipoParte" => $testigosO->getCveReferenciaTipoParte(),
                                    "idReferenciaParte" => $testigosO->getIdReferenciaParte(),
                                    "cveTipoParte" => $testigosO->getCveTipoParte(),
                                    "nombreTipoParte" => $nombreTipoParte,
                                    "nombreTestigo" => $testigosO->getNombreTestigo(),
                                    "nombreOfendido" => $nombreOfendido
                                );
                            array_push($listaTestigosO, $resultadoTestigoO);
                            }
                        }
                ...
                $idActuacion = "";
                if ($exhorto->getIdExhortoGenerado() != null) {
                    $idActuacion = 0;
                    $exhortosGeneradosDao = new ExhortosgeneradosDAO();
                    $exhortosGeneradosDto = new ExhortosgeneradosDTO();
                    $exhortosGeneradosDto->setIdExhortoGenerado($exhorto->getIdExhortoGenerado());
                    $exhortosGenerados = $exhortosGeneradosDao->selectExhortosgenerados($exhortosGeneradosDto);
                    if ($exhortosGenerados != "") {
                        $idActuacion = $exhortosGenerados[0]->getIdActuacion();
                    }
                }
                $data[$count]['idExhorto'] = $exhorto->getIdExhorto();
                $data[$count]['IdExhortoGenerado'] = $exhorto->getIdExhortoGenerado();
                $data[$count]['IdActuacion'] = $idActuacion;
                ...
                if ($exhorto->getTextoExhorto() == "" && $exhorto->getObservaciones() != "") {
                    ...
                    $data[$count]['observaciones'] = json_encode($exhorto->getTextoExhorto()) == "" ? utf8_encode($exhorto->getTextoExhorto()) : $exhorto->getTextoExhorto();
                // error_log($exhorto->getObservaciones());
//                $data[$count]['observaciones'] = ($exhorto->getObservaciones());
//                $data[$count]['observaciones'] = "hola";
                ...
                $data[$count]['listaTestigosI'] = $listaTestigosI;
                $data[$count]['listaTestigosO'] = $listaTestigosO;
    ...
    public function getTipoParte($cveTipoParte){
        ...
        return $TipospartesDTO[0]->getDescTipoParte();
    }
    ...
    public function insertExhortos($ExhortosDto, $proveedor = null, $listaImputados, $listaOfendidos, $listaDelitos,$listaTestigosI = array(),$listaTestigosO = array()) {
        $ExhortosDto = $this->validarExhortos($ExhortosDto);
        ...
        if ($contadoresDto != "") {
            ...
            if ($listaImputados != "") {
                ...
                foreach ($listaImputados as $imputado) {
                    ...
                    //SECCION PARA INSERTAR TESTIGOS DE  IMPUTADOS
                    
                    $TestigoscarpetasjudicialesDAO = new TestigoscarpetasjudicialesDAO();
                    if($listaTestigosI != null){
                        foreach ($listaTestigosI as $key => $testigo) {

                            if($imputado["cveTipoPersona"] == 1){
                                $nombreImp = $imputado["nombre"]." ".$imputado["paterno"]." ".$imputado["materno"];
                            }else{
                                $nombreImp = $imputado["nombreMoral"];
                            }
                            if($nombreImp == $testigo["nombreImputado"]){
                                if($testigo["activo"] == "S"){
                                    $TestigoscarpetasjudicialesDTO = new TestigoscarpetasjudicialesDTO();

                                    $TestigoscarpetasjudicialesDTO->setIdReferenciaParte($imputadosexhortos[0]->getIdImputadoExhorto());
                                    $TestigoscarpetasjudicialesDTO->setCveReferenciaTipoParte(8);
                                    $TestigoscarpetasjudicialesDTO->setCveTipoParte($testigo["cveTipoParte"]);
                                    $TestigoscarpetasjudicialesDTO->setNombreTestigo($testigo["nombreTestigo"]);
                                    $TestigoscarpetasjudicialesDTO->setIdCarpetaJudicial($ExhortosDto->getIdExhorto());
                                    $TestigoscarpetasjudicialesDTO->setCveTipoCarpeta($ExhortosDto->getCveTipoCarpeta());
                                    $TestigoscarpetasjudicialesDTO->setActivo("S");

                                    $TestigoscarpetasjudicialesDTO = $TestigoscarpetasjudicialesDAO->insertTestigoscarpetasjudiciales($TestigoscarpetasjudicialesDTO, $proveedor);
                                    if($TestigoscarpetasjudicialesDTO == ""){
                                        $transaccion = 0;
                                        $mensajeEx = "Error al agregar los testigos";
                                    }
                                }
                            }
                        }
            ...
            if ($listaOfendidos != "") {
                ...
                foreach ($listaOfendidos as $ofendido) {
                    ...
                    $TestigoscarpetasjudicialesDAO = new TestigoscarpetasjudicialesDAO();
                    if($listaTestigosO != null){
                        foreach ($listaTestigosO as $key => $testigoO) {

                            if($ofendido["cveTipoPersona"] == 1){
                                $nombreImp = $ofendido["nombre"]." ".$ofendido["paterno"]." ".$ofendido["materno"];
                            }else{
                                $nombreImp = $ofendido["nombreMoral"];
                            }
                            if($nombreImp == $testigoO["nombreOfendido"]){
                                if($testigoO["activo"] == "S"){
                                    $TestigoscarpetasjudicialesDTO = new TestigoscarpetasjudicialesDTO();

                                    $TestigoscarpetasjudicialesDTO->setIdReferenciaParte($ofendidosexhortos[0]->getIdOfenfendidoExhorto());
                                    $TestigoscarpetasjudicialesDTO->setCveReferenciaTipoParte(9);
                                    $TestigoscarpetasjudicialesDTO->setCveTipoParte($testigoO["cveTipoParte"]);
                                    $TestigoscarpetasjudicialesDTO->setNombreTestigo($testigoO["nombreTestigo"]);
                                    $TestigoscarpetasjudicialesDTO->setIdCarpetaJudicial($ExhortosDto->getIdExhorto());
                                    $TestigoscarpetasjudicialesDTO->setCveTipoCarpeta($ExhortosDto->getCveTipoCarpeta());
                                    $TestigoscarpetasjudicialesDTO->setActivo("S");
    ...
    public function updateExhortos($ExhortosDto, $proveedor = null, $listaImputados, $listaOfendidos, $listaDelitos,$listaTestigosI=[],$listaTestigosO=[]) {
        $idExhorto = $ExhortosDto->getIdExhorto();
        $transaccion = 1;
        $proveedor = new Proveedor('mysql', 'sigejupe');
        $proveedor->connect();
        ...
        if($listaTestigosI != null){
            foreach ($listaTestigosI as $key => $testigo) {
                ...
                if($testigo["idTestigoCarpetaJudicial"] != ""){
                    $TestigoscarpetasjudicialesDTO = new TestigoscarpetasjudicialesDTO();

                    $TestigoscarpetasjudicialesDTO->setIdTestigoCarpetaJudicial($testigo["idTestigoCarpetaJudicial"]);
                    $TestigoscarpetasjudicialesDTO->setCveTipoParte($testigo["cveTipoParte"]);
                    $TestigoscarpetasjudicialesDTO->setNombreTestigo($testigo["nombreTestigo"]);
                    $TestigoscarpetasjudicialesDTO->setActivo($testigo["activo"]);

                    $TestigoscarpetasjudicialesDTO = $TestigoscarpetasjudicialesDAO->updateTestigoscarpetasjudiciales($TestigoscarpetasjudicialesDTO, $proveedor);
                    if($TestigoscarpetasjudicialesDTO == ""){
                        $transaccion = 0;
                        $mensajeEx = "Error al agregar los testigos";
                    }
            ...
            foreach ($listaImputados as $imputado) {
                ...
                foreach ($ImputadosexhortosDto as $imputadoExistente) {
                    ...
                    if ($imputado['idImputadoExhorto'] == $imputadoExistente->getIdImputadoExhorto()) {
                        ...
                        //insertar testigos nuevos de imputados existentes
                        if($listaTestigosI != null){
                            foreach ($listaTestigosI as $key => $testigo) {
                                if($testigo["idTestigoCarpetaJudicial"] == ""){
                                    if($imputado["cveTipoPersona"] == 1){
                                    ...
                                    if($nombreImp == $testigo["nombreImputado"]){
                                        if($testigo["activo"] == "S"){
                                            ...
                                            $TestigoscarpetasjudicialesDTO->setCveTipoParte($testigo["cveTipoParte"]);
                                            $TestigoscarpetasjudicialesDTO->setNombreTestigo($testigo["nombreTestigo"]);
                                            $TestigoscarpetasjudicialesDTO->setIdCarpetaJudicial($ExhortosDto[0]->getIdExhorto());
                                            $TestigoscarpetasjudicialesDTO->setCveTipoCarpeta($ExhortosDto[0]->getCveTipoCarpeta());
                                            $TestigoscarpetasjudicialesDTO->setActivo("S");
        ...
        if($listaTestigosO != null){
            ...
            foreach ($listaTestigosO as $key => $testigoO) {
                ...
                if($testigoO["idTestigoCarpetaJudicial"] != ""){
                    $TestigoscarpetasjudicialesDTO = new TestigoscarpetasjudicialesDTO();

                    $TestigoscarpetasjudicialesDTO->setIdTestigoCarpetaJudicial($testigoO["idTestigoCarpetaJudicial"]);
                    $TestigoscarpetasjudicialesDTO->setCveTipoParte($testigoO["cveTipoParte"]);
                    $TestigoscarpetasjudicialesDTO->setNombreTestigo($testigoO["nombreTestigo"]);
                    $TestigoscarpetasjudicialesDTO->setActivo($testigoO["activo"]);
...
//        $contadoresDto->setCveJuzgado($_SESSION['cveAdscripcion']); /// variable de sesion
        $contadoresDto->setCveJuzgado($ExhortosDto->getCveJuzgado()); /// variable de sesion
        $contadoresDto->setAnio(date("Y"));
...
//                $ActuacionesDto->setCveJuzgado($_SESSION['cveAdscripcion']);
                $ActuacionesDto->setCveJuzgado($ExhortosDto->getCveJuzgado());
...
//            $ActuacionesestatusDto2 = $ActuacionesestatusDao2->selectActuacionesestatus($ActuacionesestatusDto2);
            ...
            foreach ($ActuacionesDto as $key => $actuacion) {
                ...
                foreach ($JuzgadosDto as $juzgado) {
                    ...
                    if ($actuacion->getCveJuzgado() == $juzgado->getcveJuzgado()) {
                        $desJuzgadoProcedencia = $juzgado->getDesJuzgado();
                    }
                }
                ...
                if ($ExhortosDto2) {
                    ...
                    $numeroExhorto = $ExhortosDto2[0]->getNumExhorto() . "/" . $ExhortosDto2[0]->getAniExhorto();
                } else {
                    $generado = 0;
                }
                ...
                $data[$count]['numeroExhorto'] = $numeroExhorto;
...
//                    print_r($delistosCarpetaRes);
                    if ($delistosCarpetaRes != "") {
                        foreach ($delistosCarpetaRes as $Res) {
                            ...
                            if ($delitoExhortoRes == "") {
                                $transaccion = 0;
                                error_log("delitoExhortoRes");
//                                echo "no no hizo insert..";
                            }
                        }
                    }
...
//                            print_r($imputadosEDTO);
                            ...
                            if ($imputadosERes == "") {
                                $response = [
                                    'estatus' => 'error',
                                    'mensaje' => 'Verifica que los datos de los imputados esten actualizados'
                                ];
                                error_log("imputadosERes" . $proveedor->error());
                                $transaccion = 0;
                            }
//            
                    ...
                    if ($ofendidos != "") {
                        ...
                        foreach ($ofendidos as $ofendidos) {
                            ...
                            if ($ofendidosResE == "") {
                                $response = [
                                    'estatus' => 'error',
                                    'mensaje' => 'Verifica que los datos de los ofendidos esten actualizados'
                                ];
                                $transaccion = 0;
                                error_log("OfendidosRes" . $proveedor->error());
                            }
...
//        print_r($desDelito);
                ...
                foreach ($DelitoscarpetasDto as $delito) {
                    ...
                    foreach ($DelitosDto as $del) {
                        ...
                        if ($delito->getcveDelito() == $del->getcveDelito()) {
                            ...
                            $desDelito = utf8_encode($del->getdesDelito());
                        }
                    }
    ...
    //busqueda de exhortos que no cuentan con actuaciones o audiencias y no son padres en tlbantecedescarpetas
    //para eliminar
    public function consultarExhortosaEliminar($exhortosDto, $param) {
        $ExhortosDAO = new ExhortosDAO();
        if ($exhortosDto->getNumExhorto() == "" && $exhortosDto->getAniExhorto() == "") {
            $orden.=" AND fechaRegistro >= '" . $param['fechaInicial'] . "' AND fechaRegistro <= '" . $param['fechaFinal'] . "'";
        }
        ...
        $rsExhortosDto = $ExhortosDAO->selectExhortos($exhortosDto, $orden, $proveedor);
        $total = 0;
        $data = [];
        if ($rsExhortosDto != '') { 
            $EstadosDto = new EstadosDTO();
            ...
            foreach ($rsExhortosDto as $key => $exhorto) {
                $idExhorto = $exhorto->getIdExhorto();
                ...
                // validar que no cuente con Actuaciones
                $ActuacionesDto = new ActuacionesDTO();
                $ActuacionesDao = new ActuacionesDAO();
                $ActuacionesDto->setIdReferencia($idExhorto);
                $ActuacionesDto->setCveTipoCarpeta(7);
                $ActuacionesDto->setActivo('S');
                $rsActuaciones = $ActuacionesDao->selectActuaciones($ActuacionesDto);
                
                // validar que no cuente con Audiencias
                $AudienciasDto = new AudienciasDTO();
                $AudienciasDao = new AudienciasDAO();
                $AudienciasDto->setIdReferencia($idExhorto);
                $AudienciasDto->setCveTipoCarpeta(7);
                $AudienciasDto->setActivo('S');
                $rsAudiencias = $AudienciasDao->selectAudiencias($AudienciasDto);
                
                //validar que no sea carpeta padre en AntecedesCarpetas
                $AntecedesDto = new AntecedescarpetasDTO();
                $AntecedesDao = new AntecedescarpetasDAO();
                $AntecedesDto->setIdCarpetaPadre($idExhorto);
                $AntecedesDto->setCveTipoCarpeta(7);
                $AntecedesDto->setActivo('S');
                $rsAntecedes = $AntecedesDao->selectAntecedescarpetas($AntecedesDto);
                
                $AntecedesHijoDto = new AntecedescarpetasDTO();
                $AntecedesHijoDao = new AntecedescarpetasDAO();
                $AntecedesHijoDto->setIdCarpetaHija($idExhorto);
                $AntecedesHijoDto->setCveTipoCarpeta(7);
                $AntecedesHijoDto->setActivo('S');
                $rsAntecedesH = $AntecedesHijoDao->selectAntecedescarpetas($AntecedesHijoDto);
            
                    $total ++;
                    if($rsActuaciones == '' && $rsAudiencias == '' && $rsAntecedes==''){
                        $data[$key]['eliminar'] = '1';
                    }else{
                        $data[$key]['eliminar'] = '0';
                    }
                    $data[$key]['idExhorto'] = $exhorto->getIdExhorto();
                    $data[$key]['numExhorto'] = $exhorto->getNumExhorto();
                    $data[$key]['aniExhorto'] = $exhorto->getAniExhorto();
                    $data[$key]['cveJuzgado'] = $exhorto->getCveJuzgado();
                    $data[$key]['desJuzgadoDestino'] = utf8_encode($desJuzgadoDestino);
                    $data[$key]['cveEstadoProcedencia'] = $exhorto->getCveEstadoProcedencia();
                    $data[$key]['desEstadoProcedencia'] = utf8_encode($desEstado);
                    $data[$key]['cveJuzgadoProcedencia'] = $exhorto->getcveJuzgadoProcedencia();
                    $data[$key]['desJuzgadoProcedencia'] = utf8_encode($desJuzgado);
                    $data[$key]['JuzgadoProcedencia'] = utf8_encode($exhorto->getJuzgadoProcedencia());
                    $data[$key]['carpetaInv'] = $exhorto->getCarpetaInv();
                    $data[$key]['nuc'] = $exhorto->getNuc();
                    $data[$key]['numero'] = $exhorto->getNumero();
                    $data[$key]['anio'] = $exhorto->getAnio();
                    $data[$key]['cveTipoCarpeta'] = $exhorto->getCveTipoCarpeta();
                    $data[$key]['sintesis'] = json_encode($exhorto->getSintesis()) == "" ? ($exhorto->getSintesis()) : utf8_encode($exhorto->getSintesis());
                    $data[$key]['cveEstatusExhorto'] = $exhorto->getCveEstatusExhorto();
                    
                    if($exhorto->getCveEstatusExhorto()==1){
                        $data[$key]['desEstatus'] = 'DILIGENCIADO';
                    }else{
                        $data[$key]['desEstatus'] = 'NO DILIGENCIADO';
                    }
                    
                    if($rsAntecedesH != ''){ //es carpeta hija en tblantecedes
                        $data[$key]['idAntecedeCarpeta'] = $rsAntecedesH->getIdAntecedeCarpeta();
                    }else{
                        $data[$key]['idAntecedeCarpeta'] = '';
                    }
                    $data[$key]['fechaRegistro'] = $exhorto->getFechaRegistro();
            } 
            $response = [
                'estatus' => 'ok',
                'totalCount' => $total,
