...
//        }
        ...
        if (!$validacion->num(1, 3, $OfendidoscarpetasDto->getCveTipoPersona())) {
            if ($OfendidoscarpetasDto->getCveTipoPersona() < 1 && $OfendidoscarpetasDto->getCveTipoPersona() > 5) {
                $estatus = 'error';
        ...
        } elseif ($OfendidoscarpetasDto->getCveTipoPersona() == 2 || $OfendidoscarpetasDto->getCveTipoPersona() == 3 || $OfendidoscarpetasDto->getCveTipoPersona() == 4 || $OfendidoscarpetasDto->getCveTipoPersona() == 5) {
    ...
    public function modificarOfendido($OfendidoscarpetasDto, $proveedor = '') {
        ...
        } else if ($ofendido->getCveTipoPersona() == 2 || $ofendido->getCveTipoPersona() == 3 || $ofendido->getCveTipoPersona() == 4 || $ofendido->getCveTipoPersona() == 5) {
            $ofendido->setActivo('S');
            $ofendido->setNombre('');
            $ofendido->setPaterno('');
            $ofendido->setMaterno('');
    ...
    public function eliminarOfendido($OfendidoscarpetasDto, $proveedor = null, $actualizarCarpeta = 0) {
        ...
        if ($ofendidosCarpetas != "") {
            if ($actualizarCarpeta == 0) {
                ...
                if ($numOfendidos > 1) {
                    $validaEliminar = $this->validaeliminarOfendido($OfendidoscarpetasDto, $this->proveedor);
                    if ( $validaEliminar['estatus'] == 'ok' ) {
                        ...
                        return [
                            'status' => 'error',
                            'mensaje' => $validaEliminar['mensaje']
                        ];
                    }
                } else {
                    return [
...
//                    }
                    ...
                    if ($selectDefensores == "") {
                        ...
                        /*
                         * si el tipo de persona es 1 o 4 o 5(persona fisica, menor de identidad resguardada, victima de identidad resguardada) se muestra nombre, paterno y materno
                         */
                        if ($ofendido->getCveTipoPersona() == 1) {
                            $nombre_ofendido = $ofendido->getNombre() . ' ' . $ofendido->getPaterno() . ' ' . $ofendido->getMaterno();
                        } else {
                            /*
                             * si el tipo de persona es igual a 2, 3, 4 o 5 (moral u otra) se muestra nombre moral
                             */
                            $nombre_ofendido = $ofendido->getNombreMoral();
                        }
                ...
                if ($ImpofedelcarpetasDto != "") {//echo"entra";
                    ...
                    foreach ($ImpofedelcarpetasDto as $OfendidoDelito) {
                        ...
                        if ($DelitosCarpetasDto != "") {
                            ...
                            $ImputadoDto->setIdImputadoCarpeta($OfendidoDelito->getIdImputadoCarpeta());
...
//        }                
    ...
    public function validaeliminarOfendido($ofendidoscarpetasDto, $proveedor = null) {
        if ($proveedor == null) {
            $this->proveedor = new Proveedor('mysql', 'sigejupe');
            $this->proveedor->connect();
        } else {
        ...
        /*
         * verificar si el ofendido cuenta con medidas de proteccion activas
         */
        $medidasProteccionDto = new MedidasproteccionesDTO();
        $medidasProteccionDao = new MedidasproteccionesDAO();
        $medidasProteccionDto->setActivo('S');
        $medidasProteccionDto->setIdOfendidoCarpeta($ofendidoscarpetasDto->getIdOfendidoCarpeta());
        $medidasProteccionDto = $medidasProteccionDao->selectMedidasprotecciones($medidasProteccionDto, "", $this->proveedor);
        if ( $medidasProteccionDto != "" ) {
            $eliminar = false;
            $mensaje[] = array("No se puede eliminar al sujeto pasivo del delito debido a que cuenta con medidas de proteccion activas");
        }
        /*
         * verificar si el ofendido presento alguna queja
         */
        ...
        $quejososPromocionesDto->setIdOfendidoCarpeta($ofendidoscarpetasDto->getIdOfendidoCarpeta());
        ...
        if ( $quejososPromocionesDto != "" ) {
            $eliminar = false;
            $mensaje[] = array("No se puede eliminar al sujeto pasivo del delito debido a que cuenta con promociones de queja activas");
        }
