...
//                $bolStatusMailAdm = false;
                ...
                if (!$error) {
                    ...
                    if ($origenFacade == "sistema") {
                        $SolicitudesmuestrasDto->setCaseProceedingId("0");
                    } else {
                        $SolicitudesmuestrasDto->setCaseProceedingId($param["caseProceedingId"]); #numero Carpeta Judicial
                    }
                    ...
                    $SolicitudesmuestrasDto->setCveCatAudiencia("58"); #58 - AUDIENCIA PARA SOLICITAR TOMA DE MUESTRA
                ...
                if (!$error) {
                    ...
                    $contadoresDto->setCveJuzgado(10322);
                    $contadoresDto->setCveTipoActuacion("");
                    $contadoresDto->setCveTipoCarpeta(11);
                ...
                if (!$error) {
                    if ($SolicitudesmuestrasDto->getIdSolicitudMuestra() != "" && $SolicitudesmuestrasDto->getIdSolicitudMuestra() > 0) {
                        ...
                        $RMuestraDto->setNumeroEspecializadoMuestra($contadoresDto->getNumero());
                ...
                #OBTENERMOS EL NUMERO DEL CONTADOR DE LA MUESTRA
                if (!$error) {
                    $contadoresDto = new ContadoresDTO();
...
//                    }
                        ...
                        $programacionmuestrasDto->setFechaInicio(date("Y-m-d H:i:s"));
                        $arrayJuzgador = $programacionmuestrasController->juezTomaDeMuestra($juzgadoJuezDto, $programacionmuestrasDto, $proveedor);
                        if ($arrayJuzgador != "" && $arrayJuzgador["estatus"] == "ok") {
                            $idJuezMuestra = $arrayJuzgador["data"]["idJuzgador"];
                            if ((int) $idJuezMuestra >= 1) {
                                $JuzgadorUlti = new JuzgadoresmuestrasDTO();
                                $JuzgadorUlti->setActivo("S");
                                $JuzgadorUlti->setIdJuzgador($idJuezMuestra);
                                $orden = ' ORDER BY idJuzgadorMuestra DESC limit 1 ';
                                $JuzgadorUlti = $JuzgadoresrespmuestrasDao->selectJuzgadoresmuestras($JuzgadorUlti, $orden, $proveedor);
                                $fechaRegistro != "";
                                $sinAntecedentes = false;

                                if ($JuzgadorUlti != "" && count($JuzgadorUlti) > 0) {
                                ...
                                if (!$sinAntecedentes) {
                                    if ($fechaRegistro != "") {
                                        ...
                                        $tiempoSolicitudDto->setCveTipoSolicitud(3);
                                        ...
                                        $fechaActual = $SolicitudesmuestrasDao->selectFechaHoraMySql($proveedor);
                                ...
                                $error = true;
                                $tmp = array("type" => "Error", "text" => "ERROR NO EXISTEN LOS HORARIOS DE TOMAS DE MUESTRA DEFINIDOS PARA EL JUZGADO");
                            } else if ((int) $idJuezMuestra == -1) {
                                $error = true;
                                $tmp = array("type" => "Error", "text" => "ERROR NO EXISTEN JUEZ DISPONLIBLE PARA DICHA SOLICITUD DE TOMAS DE MUESTRA DE APREHENSION");
                            }
                        } else {
                            $error = true;
                        ...
                        $nipSolicitudDto->setCveTipoCarpeta('11');
                        ...
                        if ($nipSolicitudDto != '') {
                            ...
                            $idJuezMuestra = $nipSolicitudDto->getIdJuzgador();
                        } else {
                            $error = true;
                ...
                if (!$error) {
                    ...
                    $juzgadoresmuestrasDto->setIdSolicitudMuestra($SolicitudesmuestrasDto->getIdSolicitudMuestra()); #Id de la solicitud del Muestra
                    $juzgadoresmuestrasDto->setIdJuzgador($idJuezMuestra); #Id de el juzgador seleccionado en l programacion
                    $juzgadoresmuestrasDto->setActivo("S");
                    $juzgadoresmuestrasDto = $JuzgadoresrespmuestrasDao->insertJuzgadoresmuestras($juzgadoresmuestrasDto, $proveedor);
                    if ($juzgadoresmuestrasDto != "" && count($juzgadoresmuestrasDto) > 0) {
                        
                    } else {
                        $error = true;
                        $tmp = array("type" => "Error", "text" => "ERROR AL GUARDAR LA RELACION JUZGADOR - TOMA DE MUESTRA");
                    }
                }
//                if (!$error) {
//                    $idJuezMuestra = 0;
                ...
                if (!$error) {
                    if ($nip != '') {
                        if ($nipSolicitudDto != '') {
                            ...
                            $nipUpdateDto->setCveTipoCarpeta('11');
                ...
                if (!$error) {
                    if ($origenFacade == "sistema") {
                        ...
                        if ($fileSolicitud != "") {
                            ...
                            $documentosImgDto->setIdCarpetaJudicial($RMuestraDto->getIdMuestra());
                            ...
                            if ($documentosImgDto != "") {
                                ...
                                $nombreArchivo = "TOMAMUESTRA_" . $RMuestraDto->getAnioMuestra() . $RMuestraDto->getNumeroMuestra() . ".pdf";
                                $pdf = fopen($nombreArchivo, 'w');
                                fwrite($pdf, $pdf_decoded);
                                fclose($pdf);

                                $path = "../../../imagenes"; //Nodo Raiz
                                $ruta = $path . '/' . $juzgadoSolicitar . '/' . $RMuestraDto->getAnioMuestra() . '/TOMA_DE_MUESTRA/' . $RMuestraDto->getNumeroMuestra() . "/";
                                if (!is_dir($ruta)) {
