...
class CarpetasjudicialesService {
    ...
    public function generarCarpetaDesdeSolicitud($idSolicitudAudiencia, $proveedor = null, $reclusosImputados = '')
    {
        ...
        try {
            ...
            if ($getSolicitud == '') throw new Exception('no se encontro una solicitud de audiencia con el id = ' . $idSolicitudAudiencia);
            /*
             *
             */
            ...
            if ($getCatAudiencias == '') throw new Exception('no se encontro la categoria de audiencia con id: ' . $cveCatAudienciaSolicitud . ', para obtener la etapa procesal');
            ...
            if ($getSolicitud[0]->getIdReferencia() == 0 || is_null($getSolicitud[0]->getIdReferencia())) {
                 ...
                 */
                if ($cveEtapaProcesal > 2 && $cveEtapaProcesal != 6) throw new Exception('la etapa procesal de la categoria de audiencia tiene que ser auxiliar o de control (1 o 2)');
                ...
                if ($cveEtapaProcesal != 6) {
                    ...
                    if ($getCarpetaJudicialByNucOrCarpetaInv != '') throw new Exception($mensaje);
                ...
                if ($getContador == '') throw new Exception('no se pudo generar el contador');
                ...
                if ($crearCarpetaJudicial['estatus'] == 'error') throw new Exception($crearCarpetaJudicial['mensaje']);
                ...
                if ($copiarImputados['estatus'] == 'error') throw new Exception($copiarImputados['mensaje']);
                if ($copiarOfendidos['estatus'] == 'error') throw new Exception($copiarOfendidos['mensaje']);
                if ($copiarDelitos['estatus'] == 'error') throw new Exception($copiarDelitos['mensaje']);
                if ($copiarRelacion['estatus'] == 'error') throw new Exception($copiarRelacion['mensaje']);
                if ($copiarApelantes['estatus'] == 'error') throw new Exception($copiarApelantes['mensaje']);
                ...
                if ($insertAntecedesCarpetas['estatus'] == 'error') throw new Exception($insertAntecedesCarpetas['mensaje']);
                ...
                if ($editarSolicitudAudiencia['estatus'] == 'error') throw new Exception($editarSolicitudAudiencia['mensaje']);
                ...
                if ($cveEtapaProcesal == 1) {
                        ...
                        if ($catAudiencia->getCausa() == 'S') {
                            ...
                            */
                            ...
                            if ($getCarpetaJudicial != '') {
                                ...
                                if ($updateCarpeta == '') throw new Exception('no se pudo modificar la fecha de radicación de la carpeta anterior con id : ' . $idReferencia);
                        ...
                        if ($catAudiencia->getCausa() == 'S') {
                            ...
                            */
                            ...
                            if ($ultimaEtapaProcesal == 1) { //si la etapa procesal anterior era = 1, entonces se edita la etapa procesal por la nueva que es = 2
                                ...
                                if ($updateEtapaCarpetaJudicial == '') throw new Exception('no se pudo editar la etapa procesal de la carpeta judicial: ' . $idReferencia);
                    ...
                    if ($ultimaEtapaProcesal == 3) {
                        ...
                        if ($getImputados == '') throw new Exception('no se encontraron imputados activos en el id solicitud : ' . $idSolicitudAudiencia);
                            ...
                            if ($getImputadosCarpetas != '') {
                                ...
                                foreach ($getImputadosCarpetas as $imputadoCarpeta) {
                                    ...
                                    if (($imputadoCarpeta->getCveEtapaProcesal() == $cveEtapaProcesal)) {
                                        ...
                                        if ($getCarpetaJudicialDto == '') throw new Exception('no se encontro la carpeta judicial con id: ' . $imputadoCarpeta->getIdCarpetaJudicial());
    ...
    public function creaAntecedesCarpetas($idCarpetaPadre, $idCarpetaHija, $carpetaJudicialNueva, $proveedor = null)
    {
        ...
        try {
            ...
            if ($insertAntecedesCarpetas == '') throw new Exception('no se pudo insertar en antecedescarpetas, intenta nuevamente');
    ...
    public function crearCarpetaJudicial($getSolicitud, $cveEtapaProcesal, $cveTipoCarpeta, $cveConclucion, $ponderacion, $cveJuzgado, $proveedor = null)
    {
        ...
        try {
            ...
            if (is_null($getSolicitud[0]->getCveEtapaProcesal())) throw new Exception('la solicitud de audiencia a copiar no cuenta con clave de etapa procesal.');
            if (is_null($getSolicitud[0]->getCveConsignacion())) throw new Exception('la solicitud de audiencia a copiar no cuenta con clave de consignacion');
            ...
            if ($insertarCarpetaJudicial == '') throw new Exception('no se pudo crear la carpeta judicial, intenta nuevamente');
        ...
        try {
            ...
            if ($getImputadosSolicitudes != '') {
                ...
                foreach ($getImputadosSolicitudes as $imputadoSolicitud) {
                    ...
                    if ($insertImputadoCarpeta == '') throw new Exception('no se pudo copiar el imputado con id: ' . $imputadoSolicitud->getIdImputadoSolicitud() . ' a imputadoscarpetas, ocurrio un error');
                    ...
                    if ($actualizaImputadoSolicitud == '') throw new Exception('no se pudo actualizar el idimputado carpeta en imputadosolicitud con id' . $imputadoSolicitud->getIdImputadoSolicitud());
                    ...
                    if ($getDomiciliosImputadosSolicitud != '') {
                        ...
                        foreach ($getDomiciliosImputadosSolicitud as $domicilioImputadoSolicitud) {
                            ...
                            if ($insertDomicilioImputadoCarpeta == '') throw new Exception('no se puedo guardar un domicilio de un imputado, intenta nuevamente');
                    ...
                    if ($getTelefonosImputadoSolicitud != '') {
                        ...
                        foreach ($getTelefonosImputadoSolicitud as $telefonoImputadoSolicitud) {
                            ...
                            if ($insertTelefonoImputadoCarpeta == '') throw new Exception('no se puedo insertar un telefono de un imputado, intenta nuevamente');
                    ...
                    if ($getDefensoresImputadosSolicitud != '') {
                        ...
                        foreach ($getDefensoresImputadosSolicitud as $defensorImputadoSolicitud) {
                            ...
                            if ($insertDefensorImputadoCarpeta == '') throw new Exception('no se pudo insertar un defensor de imputado, intenta nuevamente');
                    ...
                    if ($getTutoresImputadosSolicitud != '') {
                        ...
                        foreach ($getTutoresImputadosSolicitud as $tutorImputadoSolicitud) {
                            ...
                            if ($insertTutorImputadoCarpeta == '') throw new Exception('no se pudo guardar un tutor de imputado, intenta nuevamente');
                    ...
                    if ($getDrogasimputadosSolicitud != '') {
                        ...
                        foreach ($getDrogasimputadosSolicitud as $drogaImputadoSolicitud) {
                            ...
                            if ($insertDrogaImputadoCarpeta == '') throw new Exception('no se pudo guardar una droga de imputado, intenta nuevamente');
                    ...
                    if ($getNacionalidadesImputadosSolicitud != '') {
                        ...
                        foreach ($getNacionalidadesImputadosSolicitud as $nacionalidadImputadoSolicitud) {
                            ...
                            if ($insertNacionalidadImputadoCarpeta == '') throw new Exception('no se pudo guardar una nacionalidad de imputado, intenta nuevamente');
                        }
                    }
            ...
            if (isset($_SESSION['imputados_copia'])) unset($_SESSION['imputados_copia']);
    ...
    public function CopiarOfendidos($idSolicitudAudiencia, $idCarpetaJudicial, $proveedor = null, $byIdOfendido = null)
    {
        ...
        try {
                ...
                foreach ($getOfendidosSolicitudes as $ofendidoSolicitud) {
                    ...
                    if (count($getDomiciliosOfendidosSolicitud)) {
                        ...
                        foreach ($getDomiciliosOfendidosSolicitud as $domicilioOfendidoSolicitud) {
                            ...
                            if ($insertDomicilioOfendidoCarpeta == '') throw new Exception('no se pudo copiar el domicilio del ofendido con id: ' . $domicilioOfendidoSolicitud->getIdDomicilioOfendidoSolicitud() . ', intenta nuevamente');
                    ...
                    if (count($getTelefonosOfendidoSolicitud)) {
                        ...
                        foreach ($getTelefonosOfendidoSolicitud as $telefonoOfendidoSolicitud) {
                            ...
                            if ($insertTelefonoOfendidoCarpeta == '') throw new Exception('no se puedo insertar un telefono de un ofendido, intenta nuevamente');
                    ...
                    if (count($getDefensoresOfendidosSolicitud)) {
                        ...
                        foreach ($getDefensoresOfendidosSolicitud as $defensorOfendidoSolicitud) {
                            ...
                            if ($insertDefensorOfendidoCarpeta == '') throw new Exception('no se pudo insertar un defensor de ofendido, intenta nuevamente');
                    ...
                    if ($getTutoresOfendidoSolicitud != '') {
                        ...
                        foreach ($getTutoresOfendidoSolicitud as $tutorOfendidoSolicitud) {
                            ...
                            if ($insertTutorOfendidoCarpeta == '') throw new Exception('no se pudo guardar un tutor de ofendido, intenta nuevamente');
                    ...
                    if (count($getNacionalidadesOfendidosSolicitud)) {
                        ...
                        foreach ($getNacionalidadesOfendidosSolicitud as $nacionalidadOfendidoSolicitud) {
                            ...
                            if ($insertNacionalidadImputadoCarpeta == '') throw new Exception('no se pudo guardar una nacionalidad del ofendido, intenta nuevamente');
                        }
                    }
            ...
            if (isset($_SESSION['ofendidos_copia'])) unset($_SESSION['ofendidos_copia']);
        ...
        try {
            ...
            if ($getDelitosSolicitud != '') {
                ...
                foreach ($getDelitosSolicitud as $delitoSolicitud) {
                    ...
                    if ($insertDelitoCarpeta == '') throw new Exception('no se pudo copiar un delito de la solicitud a carpetas judiciales, intenta nuevamente');
            ...
            if (isset($_SESSION['delitos_copia'])) unset($_SESSION['delitos_copia']);
        ...
        try {
            ...
            if ($getImoOfeDelSolicitud != '') {
                ...
                foreach ($getImoOfeDelSolicitud as $relacionSolicitud) {
                    ...
                    if (!isset($_SESSION['imputados_copia'][$relacionSolicitud->getIdImputadoSolicitud()])) throw new Exception('la variable de sesión no esta inicializada, no se encontro la referencia del imputado de solicitud ' . $relacionSolicitud->getIdImputadoSolicitud());
                    if (!isset($_SESSION['ofendidos_copia'][$relacionSolicitud->getIdOfendidoSolicitud()])) throw new Exception('la variable de sesión no esta inicializada, no se encontro la referencia del ofendido de solicitud ' . $relacionSolicitud->getIdOfendidoSolicitud());
                    if (!isset($_SESSION['delitos_copia'][$relacionSolicitud->getIdDelitoSolicitud()])) throw new Exception('la variable de sesión no esta inicializada, no se encontro la referencia del delito de solicitud ' . $relacionSolicitud->getIdDelitoSolicitud());
                    ...
                    if ($insertRelacionCarpeta == '') throw new Exception('no se pudo copiar una relación a carpetas judiciales');
                            ...
                            */
                            ...
                            if ($getViolenciaGeneroSolicitud != '') {
                                ...
                                foreach ($getViolenciaGeneroSolicitud as $violenciaGeneroSolicitud) {
                                    ...
                                    if ($insertViolenciaGeneroCarpeta == '') throw new Exception('no se pudo copiar violencia genero a carpetas judiciales');
                                    ...
                                    if ($getViolenciaFactoresSocialesSolicitud != '') {
                                        ...
                                        foreach ($getViolenciaFactoresSocialesSolicitud as $violenciaFactorSocialSolicitud) {
                                            ...
                                            if ($insertViolenciaFactorSocialCarpeta == '') throw new Exception('no se pudo copiar violencia factores sociales a carpetas judiciales');
                                        }
                                    ...
                                    if ($getViolenciaHomicidiosDolososSolicitud != '') {
                                        ...
                                        foreach ($getViolenciaHomicidiosDolososSolicitud as $violenciaHomicidiosDolososSolicitud) {
                                            ...
                                            if ($insertViolenciaHomicidioDolosoCarpeta == '') throw new Exception('no se pudo copiar homicidios dolosos a carpetas judiciales');
                            ...
                            if ($insertEfectosOfendidosCarpetas == '') throw new Exception('no se pudo copiar un efecto ofendido a carpetas judiciales');
                    ...
                    if ($getTrataPersonasSolicitud != '') {
                        ...
                        foreach ($getTrataPersonasSolicitud as $trataPersonaSolicitud) {
                            ...
                            if ($insertTrataPersonaCarpeta == '') throw new Exception('no se pudo copiar trata de personas a carpetas judiciales');
                    ...
                    if ($getSexualesSolicitud != '') {
                        ...
                        foreach ($getSexualesSolicitud as $sexualSolicitud) {
                            ...
                            if ($insertSexualCarpeta == '') throw new Exception('no se pudo copiar sexuales a carpetas judiciales');
                            ...
                            if ($getTestigosSexualesSolicitud != '') {
                                ...
                                foreach ($getTestigosSexualesSolicitud as $testigoSexualSolicitud) {
                                    ...
                                    if ($insertTestigoSexualCarpeta == '') throw new Exception('no se pudo copiar testigos sexuales a carpetas judiciales');
                            ...
                            if ($getSexualesConductasSolicitud != '') {
                                ...
                                foreach ($getSexualesConductasSolicitud as $sexualConductaSolicitud) {
                                    ...
                                    if ($insertSexualConductaCarpeta == '') throw new Exception('no se pudo copiar sexuales conductas a carpetas judiciales');
                                    ...
                                    if ($getDetalleSexualesConductasSolicitud != '') {
                                        ...
                                        foreach ($getDetalleSexualesConductasSolicitud as $detalleSexualConductaSolicitud) {
                                            ...
                                            if ($insertDetalleSexualConductaCarpeta == '') throw new Exception('no se pudo copiar detalles sexuales conductas a carpetas judiciales');
    ...
    public function getPonderacion($idSolicitudAudiencia, $proveedor = null, $byIdImputadoSolicitud = null)
    {
        ...
        if ($getImpofedelSolicitud == '') throw new Exception('no se encontro la relacion en impofedelsolicitud y no se puede calcular la ponderacion');
        ...
        foreach ($getImpofedelSolicitud as $impofedelSolicitud) {
            ...
            if ($getDelitoSolicitud == '') throw new Exception('no se encontro el delitosolicitud con id ' . $impofedelSolicitud->getIdDelitoSolicitud());
            ...
            if ($getDelito == '') throw new Exception('no se encontro el delito con el id ' . $getDelitoSolicitud[0]->getCveDelito());
    ...
    public function updateSolicitudAudiencia($getSolicitud, $carpetaJudicial, $proveedor = null)
    {
        try {
            ...
            if ($updateSolicitudAudiencias == '') throw new Exception('no se pudo editar el id de referencia en la solicitud de audiencias');
            /*
    ...
    public function cerrarCarpeta($idCarpetaReferencia, $idCarpetaNueva, $proveedor = null)
    {
        try {
            ...
            if ($getCarpetaReferencia == '') throw new Exception('no se encontró la carpeta de referencia para cerrar');
            if ($getCarpetaNueva == '') throw new Exception('no se encontro la nueva carpeta para cerrar');
            ...
            if ($imputadosCarpetaReferencia == $imputadosCarpetaNueva) {
                ...
                if ($updateCarpetaReferencia == '') throw new Exception('no se pudo cerrar la carpeta de referencia, error al modificar los datos');
            }
    ...
    public function modificarEtapaProcesalImputadosCerrarCarpeta($nuevaEtapaProcesal, $idSolicitudAudiencia, $proveedor = null)
    {
        ...
        if ($getImputadosSolicitud == '') throw new Exception('no se encontraron imputados en la solicitud');
        ...
        foreach ($getImputadosSolicitud as $imputadoSolicitud) {
            ...
            if ($modificarEtapaImputado == '') throw new Exception('no se pudo modificar la nueva etapa procesal del imputadocarpeta id = ' . $imputadoSolicitud->getIdImputadoCarpeta());
        ...
        if ($getCarpeta == '') throw new Exception('no se encontro la carpeta judicial id:' . $idCarpetaJudicialImputadoCarpeta . ', para intentar cerrar la carpeta');
        ...
        if ($getImputadosCarpetas == '') throw new Exception('no se encontraron imputados');
        ...
        foreach ($getImputadosCarpetas as $imputadoCarpeta) {
            ...
            if ($tipoCarpeta == 1) {
                ...
                if ($imputadoCarpeta->getCveEtapaProcesal() > $etapaProcesalCarpeta) $countCerrarCarpeta ++;
            }
                    ...
                    if ($selectAperturasJuicio != '') {
                        ...
                        foreach($selectAperturasJuicio as $aperturaJuicioDto){
                            ...
                            if($aperturaJuicioDto->getIdAudienciaJuicio() == ''){
                                ...
                                if ($modificarAperturaJuicio == '') throw new Exception('No se pudo modificar el auto de apertura a juicio con id apertura juicio: ' . $idAperturaJuicio);
        ...
        if (count($arrayRelacion)) {
            ...
            if (count($relacionReclusos)) {
                ...
                foreach ($arrayRelacion as $relacion) {
                    ...
                    if (!isset($relacionReclusos[$relacion['idImputadoSolicitud']])) throw new Exception('No se encontro el idImputadoSolicitud en el array de relacion impsolicitud - impcarpeta');

                    $reclusosDto->setIdRecluso($relacion['idRecluso']);

                    $selectRecluso = $reclusosDao->selectReclusos($reclusosDto, '', $proveedor);

                    if ($selectRecluso == '') throw new Exception('No se encontro el recluso con id ' . $relacion['idRecluso']);
                    ...
                    if ($actualizaRecluso == '') throw new Exception('No se pudo actualizar un recluso');
